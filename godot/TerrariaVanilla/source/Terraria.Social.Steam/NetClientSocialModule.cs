...
public class NetClientSocialModule : NetSocialModule
{
	...
	{
		base.Initialize();
		_gameLobbyJoinRequested = Callback<GameLobbyJoinRequested_t>.Create((Callback<GameLobbyJoinRequested_t>.DispatchDelegate)OnLobbyJoinRequest);
		_p2pSessionRequest = Callback<P2PSessionRequest_t>.Create((Callback<P2PSessionRequest_t>.DispatchDelegate)OnP2PSessionRequest);
		_p2pSessionConnectfail = Callback<P2PSessionConnectFail_t>.Create((Callback<P2PSessionConnectFail_t>.DispatchDelegate)OnSessionConnectFail);
	...
	public override void LaunchLocalServer(Process process, ServerMode mode)
	{
		...
		if ((mode & ServerMode.Lobby) != 0)
		{
			_hasLocalHost = true;
			if ((mode & ServerMode.FriendsCanJoin) != 0)
			{
			...
			if ((mode & ServerMode.FriendsOfFriends) != 0)
			{
	...
	private void OnLobbyEntered(LobbyEnter_t result, bool failure)
	{
		...
		while (SteamNetworking.GetP2PSessionState(_lobby.Owner, out val) && val.m_bConnectionActive != 1)
		{
	...
	private void SendAuthTicket(CSteamID address)
	{
		...
		if (_authTicket == HAuthTicket.Invalid)
		{
			...
			val.SetSteamID(address);
			_authTicket = SteamUser.GetAuthSessionTicket(_authData, _authData.Length, out _authDataLength, ref val);
		}
