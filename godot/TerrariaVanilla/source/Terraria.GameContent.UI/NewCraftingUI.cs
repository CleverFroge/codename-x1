...
public class NewCraftingUI : UIState
{
	...
	private static readonly Color DisabledSlotColor = new Color((byte)160, (byte)160, (byte)160, (byte)255);
	...
	private void BuildInfinitesMenuContents(UIElement totalContainer)
	{
		...
		UIHorizontalSeparator uIHorizontalSeparator = new UIHorizontalSeparator
		{
			...
			HAlign = 0.5f,
			Color = new Color((byte)89, (byte)116, (byte)213, (byte)255) * 0.9f
		};
	...
	internal static void AddTooltipLines(Item hoverItem, ref int numLines, string[] lineText, Color[] lineColors)
	{
		if (_instance == null || (_instance._missingRequirementsTooltipText != null && !hoverItem.IsAir))
		{
			...
			lineColors[numLines] = new Color((byte)255, (byte)140, (byte)160, (byte)255);
			numLines++;
		}
	}
	...
	private void DrawOwnedItemCount(SpriteBatch spriteBatch, int owned, bool enough, Vector2 mpos, float mscale)
	{
		...
		Color baseColor = (enough ? new Color((byte)144, (byte)238, (byte)144, (byte)255) : new Color((byte)255, (byte)140, (byte)160, (byte)255));
	...
	private void SetupGamepadPoints(bool craftSlotVisible, int materialCount)
	{
		...
		{
			UILinkPoint upSide = ((i == 10) ? uILinkPoint : uILinkPage2.LinkMap[40 + (int)Math.Round((float)(i * 10) / 11f)]);
			_helper.PairUpDown(upSide, array[i]);
			array[i].Down = uILinkPoint7.ID;
		}
		int num = 0;
		if (uILinkPoint4 != null)
		{
			_helper.PairUpDown(uILinkPoint4, array[num]);
			_helper.PairUpDown(uILinkPage2.LinkMap[40], uILinkPoint4);
			num++;
		}
		_helper.PairLeftRight(uILinkPoint4, uILinkPoint5);
		_helper.PairUpDown(uILinkPoint5, array[num]);
		_helper.PairUpDown(uILinkPage2.LinkMap[40 + num], uILinkPoint5);
		num++;
		_helper.PairLeftRight(uILinkPoint5, uILinkPoint6);
		if (uILinkPoint6 != null)
		{
			_helper.PairUpDown(uILinkPoint6, array[num]);
			_helper.PairUpDown(uILinkPage2.LinkMap[40 + num], uILinkPoint6);
		}
		...
		if (orderedPointsByCategoryName2.Count > 0)
		{
			...
			{
				int num2 = SelectedEntry.gridIndex - orderedPointsByCategoryName2[0].Id;
				if (num2 >= 0 && num2 < orderedPointsByCategoryName2.Count)
				{
					uILinkPoint9 = array2[num2 % itemsPerLine, num2 / itemsPerLine];
		...
		{
			UILinkPoint uILinkPoint11 = uILinkPage.LinkMap[20001 + j];
			_helper.PairLeftRight(uILinkPoint11, uILinkPage.LinkMap[20001 + j + 1]);
	...
	public override void OnActivate()
	{
		...
		_selectedRecipeIndex = ((Main.numAvailableRecipes < 0) ? null : new int?(Main.availableRecipe[Main.focusRecipe]));
		ResetRecipes();
