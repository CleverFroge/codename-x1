...
public static class ChatManager
{
	...
	public static IEnumerable<PositionedSnippet> LayoutSnippets(DynamicSpriteFont font, IEnumerable<TextSnippet> snippets, Vector2 scale, float maxWidth = -1f)
	{
		...
		{
			if (snippet.UniqueDraw(justCheckingSize: true, out var size2, null, default(Vector2), default(Color), uniqueDrawScale))
			{
	...
	{
		DrawColorCodedString(spriteBatch, font, LayoutSnippets(font, snippets, scale, maxWidth), position, rotation, origin, scale, out hoveredSnippet, ignoreColors ? new Color?(baseColor) : null);
	}
