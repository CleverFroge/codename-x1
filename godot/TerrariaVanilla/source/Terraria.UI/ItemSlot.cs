...
public class ItemSlot
{
	...
	{
		AddPulseEffect(slot, new Color((byte)250, (byte)40, (byte)40, (byte)255));
	}
	...
	public static void Draw(SpriteBatch spriteBatch, Item[] inv, int context, int slot, Vector2 position, Color lightColor = default(Color))
	{
		...
		else
		{
			...
			case 15:
				...
				break;
			case 29:
				color2 = new Color((byte)53, (byte)69, (byte)127, (byte)255);
				value = TextureAssets.InventoryBack18.Value;
				break;
			case 34:
				color2 = new Color((byte)25, (byte)44, (byte)65, (byte)180) * 0.9f;
				value = TextureAssets.InventoryBack18.Value;
				break;
			case 30:
				flag3 = !flag2;
			...
			case 43:
				...
				{
					color2 = new Color((byte)20, (byte)40, (byte)60, (byte)180) * 0.9f;
					color2 = new Color((byte)16, (byte)36, (byte)56, (byte)180) * 0.9f;
		...
		if ((context == 0 || context == 2) && inventoryGlowTime[slot] > 0 && !inv[slot].favorited && !inv[slot].IsAir)
		{
			...
			Color value2 = new Color((byte)63, (byte)65, (byte)151, (byte)255) * num6;
		...
		if ((context == 4 || context == 32 || context == 3) && inventoryGlowTimeChest[slot] > 0 && !inv[slot].favorited && !inv[slot].IsAir)
		{
			...
			Color value4 = new Color((byte)130, (byte)62, (byte)102, (byte)255) * num8;
			if (context == 3)
			{
				value4 = new Color((byte)104, (byte)52, (byte)52, (byte)255) * num8;
			}
		...
		if (ShouldHighlightSlotForMouseItem(context, slot, Main.mouseItem))
		{
			...
			{
				color4 = new Color((byte)50, (byte)106, (byte)46, color2.A);
			}
			else if (value == TextureAssets.InventoryBack8.Value)
			{
				color4 = new Color((byte)46, (byte)106, (byte)98, color2.A);
			}
			else if (value == TextureAssets.InventoryBack12.Value)
			{
				color4 = new Color((byte)45, (byte)85, (byte)105, color2.A);
			}
		...
		if (flag4)
		{
			ItemDisplayKey itemDisplayKey = default(ItemDisplayKey);
			itemDisplayKey.Context = context;
			itemDisplayKey.Slot = slot;
			ItemDisplayKey key = itemDisplayKey;
		...
		else if (context == 6)
		{
			...
			spriteBatch.Draw(value11, position5, null, new Color((byte)100, (byte)100, (byte)100, (byte)100), 0f, default(Vector2), inventoryScale, SpriteEffects.None, 0f);
		}
	...
	public static float DrawItemIcon(Item item, int context, SpriteBatch spriteBatch, Vector2 screenPositionForItemCenter, float scale, float sizeLimit, Color environmentColor, float itemFade = 1f, bool flip = false)
	{
		...
		if (item.glowMask != -1 && item.type != 3779 && item.type != 46 && item.type != 5462)
		{
			...
			case 5146:
				...
				break;
			case 5462:
				color2 = new Color((byte)255, (byte)140, (byte)0, (byte)5);
				break;
			case 5669:
			{
			...
			case 3858:
				color2 = new Color((byte)255, (byte)255, (byte)255, (byte)63) * 0.75f;
	...
	public static void DrawMoney(SpriteBatch sb, string text, float shopx, float shopy, int[] coinsArray, bool horizontal = false, bool fromSavings = false)
	{
		...
		CoinSlot.CoinDrawState drawState = default(CoinSlot.CoinDrawState);
