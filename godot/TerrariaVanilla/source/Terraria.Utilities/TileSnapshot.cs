...
public static class TileSnapshot
{
	...
	public struct TileStruct
	{
		...
		{
			TileStruct result = default(TileStruct);
			result._type = tile.type;
			...
			else
			{
				...
				if ((result._sTileHeader & 0x7400u) != 0 && !TileID.Sets.SaveSlopes[result._type])
				{
		...
		{
			tile.type = _type;
			tile.wall = (ushort)(_wall_bTileHeader3_packed & 0x1FFFu);
			...
			tile.bTileHeader2 = 0;
			tile.bTileHeader3 = (byte)((uint)(_wall_bTileHeader3_packed >> 8) & 0xE0u);
		}
		...
		{
			TileStruct result = default(TileStruct);
			result._i0 = reader.ReadInt32();
			result._i1 = reader.ReadInt32();
			result._i2 = reader.ReadInt32();
			return result;
		}
	}
