...
public struct VoidLensHelper
{
	...
	public void DrawToDrawData(List<DrawData> drawDataList, int selectionMode)
	{
		...
		color *= _opacity;
		DrawData drawData = new DrawData(val.Value, _position - Main.screenPosition, rectangle, color, 0f, rectangle.Size() / 2f, 1f, SpriteEffects.None);
		drawDataList.Add(drawData);
		for (float num = 0f; num < 1f; num += 0.34f)
		{
			DrawData item = drawData;
			item.color = new Color((byte)127, (byte)50, (byte)127, (byte)0) * _opacity;
			item.scale *= 1.1f;
