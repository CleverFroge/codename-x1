...
public class UIManageControls : UIState
{
	...
	public override void OnInitialize()
	{
		...
		_buttonStyle = new UIKeybindingSimpleListItem(() => controllerGlyphStyle(), new Color((byte)73, (byte)94, (byte)171, (byte)255) * 0.9f);
		...
		_buttonProfile = new UIKeybindingSimpleListItem(() => PlayerInput.CurrentProfile.ShowName, new Color((byte)73, (byte)94, (byte)171, (byte)255) * 0.9f);
	...
	public UIElement CreatePanel(string bind, InputMode currentInputMode, Color color)
	{
		...
		case "sp9":
		{
			...
			uIKeybindingSimpleListItem4.OnLeftClick += delegate
			{
				...
				PlayerInput.CurrentProfile.CopyGameplaySettingsFrom(PlayerInput.OriginalProfiles[copyableProfileName2], currentInputMode);
			};
		...
		case "sp10":
		{
			...
			uIKeybindingSimpleListItem.OnLeftClick += delegate
			{
				...
				PlayerInput.CurrentProfile.CopyHotbarSettingsFrom(PlayerInput.OriginalProfiles[copyableProfileName5], currentInputMode);
			};
	...
	private void SetupGamepadPoints(SpriteBatch spriteBatch)
	{
		...
		for (int j = 0; j < snapPoints.Count; j++)
		{
			...
			if (snapPoints[j].Name == "Thin")
			{
				...
				uILinkPoint8.Down = ((j < snapPoints.Count - 2) ? (key + 2) : num);
				uILinkPoint8.Up = ((j < 2) ? 3001 : ((snapPoints[j - 1].Name == "Wide") ? (key - 1) : (key - 2)));
				...
				if (j < snapPoints.Count)
				{
					...
					uILinkPoint9.Down = ((j >= snapPoints.Count - 1) ? num : ((snapPoints[j + 1].Name == "Wide") ? (key + 1) : (key + 2)));
					uILinkPoint9.Up = ((j < 2) ? 3001 : (key - 2));
					UILinkPointNavigator.Shortcuts.FANCYUI_HIGHEST_INDEX = key;
				}
			}
			else
			{
				...
				UILinkPointNavigator.SetPosition(key, snapPoints[j].Position);
				uILinkPoint10.Down = ((j < snapPoints.Count - 1) ? (key + 1) : num);
				uILinkPoint10.Up = ((j < 1) ? 3001 : ((snapPoints[j - 1].Name == "Wide") ? (key - 1) : (key - 2)));
				UILinkPointNavigator.Shortcuts.FANCYUI_HIGHEST_INDEX = key;
				UILinkPointNavigator.Points[num].Up = key;
			}
		}
