...
public class UICharacterCreation : UIState, IHaveBackButtonCommand
{
	...
	private void MakeClothStylesMenu(UIElement middleInnerPanel)
	{
		...
		UIHorizontalSeparator element = new UIHorizontalSeparator
		{
			...
			Color = Color.Lerp(Color.White, new Color((byte)63, (byte)65, (byte)151, (byte)255), 0.85f) * 0.9f
		};
	...
	private void SetupGamepadPoints(SpriteBatch spriteBatch)
	{
		...
		case CategoryId.CharInfo:
		{
			...
			{
				UILinkPoint andSet7 = GetAndSet(num3, list2[num7]);
		...
		case CategoryId.HairStyle:
		{
			...
			{
				SnapPoint snapPoint4 = list2[num6];
		...
		default:
		{
			...
			{
				UILinkPoint andSet4 = GetAndSet(num3, list5[m]);
				andSet4.Up = num2 + 2;
			...
			{
				UILinkPoint andSet5 = GetAndSet(num3, list2[n]);
		...
		case CategoryId.Clothing:
		{
			...
			{
				UILinkPoint andSet = GetAndSet(num3, list4[j]);
				andSet.Up = num2 + j + 2;
				andSet.Down = uILinkPoint4.ID;
				if (j >= 3)
				{
					andSet.Up = num5 + (j - 3) + 1;
			...
			{
				UILinkPoint andSet2 = GetAndSet(num3, list3[k]);
				andSet2.Up = num2 + k + 5;
				andSet2.Down = num4 + ((int)MathHelper.Clamp(k, 1f, 4f) - 1) + 3;
