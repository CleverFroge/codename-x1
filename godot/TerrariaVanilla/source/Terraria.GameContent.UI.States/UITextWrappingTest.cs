...
public class UITextWrappingTest : UIState
{
	...
	private UIElement MakeElement(string value)
	{
		...
		if (mode == Mode.UIText)
		{
			UIText text2 = new UIText(value, (float)scale / 100f)
			{
				TextOriginX = 0f,
				HAlign = 0f,
				VAlign = 0f,
			...
			{
				container.Height = new StyleDimension(text2.MinHeight.Pixels, 0f);
			};
			container.Append(text2);
		}
		else
		{
			TestElement text = new TestElement(value, (float)scale / 100f, mode)
			{
				Width = StyleDimension.FromPercent(1f)
			};
			TestElement testElement = text;
