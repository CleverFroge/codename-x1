...
public class UIWorldGenDebug : UIState
{
	...
	private class GenPassElement : UIPanel
	{
		internal struct PassIconEntry
		{
			...
			internal static PassIconEntry FromBestiaryIcon(int index)
			{
				...
				PassIconEntry result = default(PassIconEntry);
				result.Icon = text;
				result.Region = tex.Frame(16, 5, index % 16, index / 16);
				result.Width = 26;
				result.Height = 26;
				return result;
			}
			...
			internal static PassIconEntry FromItem(int index)
			{
				...
				result.Width = (int)((float)region.Width * num2);
				result.Height = (int)((float)region.Height * num2);
				return result;
			}
	...
	public UIWorldGenDebug()
	{
		...
		{
			WorldGenerator.Controller controller2 = Controller;
			controller2.Paused = !controller2.Paused;
