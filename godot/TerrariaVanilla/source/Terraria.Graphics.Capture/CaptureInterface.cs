...
public class CaptureInterface
{
	...
	private abstract class CaptureInterfaceMode
	{
		...
		{
			sb.End();
			sb.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend, SamplerState.LinearClamp, DepthStencilState.None, RasterizerState.CullCounterClockwise, null, SelectionZoomMatrix);
			SetZoom_Context();
		}
	...
	private class ModeDragBounds : CaptureInterfaceMode
	{
		...
		private void DrawBound(SpriteBatch sb, Rectangle r, int mode)
		{
			...
			case 0:
				...
				break;
			case 1:
				sb.Draw(destinationRectangle: new Rectangle(r.X - 2, r.Y, r.Width + 4, r.Height), texture: TextureAssets.MagicPixel.Value, color: Color.White);
				sb.Draw(destinationRectangle: new Rectangle(r.X, r.Y - 2, r.Width, r.Height + 4), texture: TextureAssets.MagicPixel.Value, color: Color.White);
				sb.Draw(TextureAssets.MagicPixel.Value, r, Color.White);
				break;
			case 2:
				sb.Draw(destinationRectangle: new Rectangle(r.X - 2, r.Y, r.Width + 4, r.Height), texture: TextureAssets.MagicPixel.Value, color: Color.Gold);
				sb.Draw(destinationRectangle: new Rectangle(r.X, r.Y - 2, r.Width, r.Height + 4), texture: TextureAssets.MagicPixel.Value, color: Color.Gold);
	...
	private class ModeChangeSettings : CaptureInterfaceMode
	{
		...
		private void DrawWaterChoices(SpriteBatch spritebatch, Point start, Point mouse)
		{
			...
			for (int i = 0; i < 2; i++)
			{
				for (int j = 0; j < 7; j++)
				{
					...
					if (num3 % 2 == 1)
					{
						spritebatch.Draw(TextureAssets.MagicPixel.Value, r.TopLeft(), new Rectangle(0, 0, 1, 1), Color.Gold, 0f, Vector2.Zero, new Vector2(20f), SpriteEffects.None, 0f);
					}
					else
					{
						spritebatch.Draw(TextureAssets.MagicPixel.Value, r.TopLeft(), new Rectangle(0, 0, 1, 1), Color.White * num4, 0f, Vector2.Zero, new Vector2(20f), SpriteEffects.None, 0f);
					}
		...
		public override void Draw(SpriteBatch sb)
		{
			...
			EndDrawingSelection(sb);
			Rectangle rect = GetRect();
			Utils.DrawInvBG(sb, rect, new Color((byte)63, (byte)65, (byte)151, (byte)255) * 0.485f);
			for (int i = 0; i < 7; i++)
			{
				string key = "";
				string value = "";
				...
				ChatManager.DrawColorCodedStringWithShadow(sb, FontAssets.ItemStack.Value, key, rect.TopLeft() + new Vector2(20f, 20 + 20 * i), baseColor, 0f, Vector2.Zero, Vector2.One);
				ChatManager.DrawColorCodedStringWithShadow(sb, FontAssets.ItemStack.Value, value, rect.TopRight() + new Vector2(-20f, 20 + 20 * i), baseColor, 0f, FontAssets.ItemStack.Value.MeasureString(value) * Vector2.UnitX, Vector2.One);
			}
	...
	public static Rectangle FullScreenArea()
	{
		...
		try
		{
			...
			Point point = new Point((int)Math.Ceiling(vector.X), (int)Math.Ceiling(vector.Y));
			Point point2 = new Point((int)Math.Floor(vector2.X), (int)Math.Floor(vector2.Y));
	...
	private void DrawCameraLock(SpriteBatch sb)
	{
		...
		sb.Draw(TextureAssets.MagicPixel.Value, new Rectangle(0, 0, Main.screenWidth, Main.screenHeight), new Rectangle(0, 0, 1, 1), Color.Black * (CameraFrame / 5f));
		if (CameraFrame != 5f)
		{
			return;
		}
		...
		if (!(num <= 0f))
		{
			...
			ChatManager.DrawColorCodedStringWithShadow(sb, FontAssets.DeathText.Value, text, new Vector2(Main.screenWidth, Main.screenHeight) / 2f + vector3, Color.White * num, 0f, Vector2.Zero, Vector2.One);
			ChatManager.DrawColorCodedStringWithShadow(sb, FontAssets.DeathText.Value, text2, new Vector2(Main.screenWidth, Main.screenHeight) / 2f + vector4, Color.White * num, 0f, Vector2.Zero, Vector2.One);
		}
	}

	public static void StartCamera(CaptureSettings settings)
	{
		settings.CameraSpaceEffects = FullScreenArea().Contains(settings.Area);
