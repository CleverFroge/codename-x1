...
public class WaterfallManager
{
			...
			else
			{
				...
				case 26:
				{
					...
					for (int j = 0; j < num22; j++)
					{
						...
						if (num12 == 26)
						{
							color = new Color((byte)255, (byte)255, (byte)255, (byte)127);
					...
					case 1:
						...
						if (num30 == 1)
						{
							for (int m = 0; m < 8; m++)
							{
								...
								DrawWaterfall(num12, num13, num14, alpha, new Vector2(num13 * 16 + num48, num14 * 16 + num11 + num50) - Main.screenPosition, new Rectangle(16 + num21 + num49, 0, 2, 16 - num11), color5, SpriteEffects.FlipHorizontally);
							}
						}
						else
						{
					...
					case -1:
						...
						if (num30 == -1)
						{
							for (int l = 0; l < 8; l++)
							{
								...
								DrawWaterfall(num12, num13, num14, alpha, new Vector2(num13 * 16 + num45, num14 * 16 + num11 + num47) - Main.screenPosition, new Rectangle(16 + num21 + num46, 0, 2, 16 - num11), color5, SpriteEffects.FlipHorizontally);
							}
						}
						else
						{
	...
	private void DrawWaterfall(int waterfallType, int x, int y, float opacity, Vector2 position, Rectangle sourceRect, Color color, SpriteEffects effects)
	{
		...
		{
			Main.tileBatch.Draw(value, position, sourceRect, color, default(Vector2), 1f, effects);
		}
	}
	...
	private static float GetAlpha(float Alpha, int maxSteps, int waterfallType, int y, int s, Tile tileCache)
	{
		float num = waterfallType switch
		{
			...
			14 => 0.8f, 
			25 => 0.75f, 
			_ => ((tileCache.wall != 0 && (_shouldShowInvisibleBlocksAndWalls || !tileCache.invisibleWall())) || !((double)y < Main.worldSurface)) ? (0.6f * Alpha) : Alpha, 
		};
		if (s > maxSteps - 10)
		{
