...
public class PopupText
{
	...
	private static void EmitFancyFlashDust(PopupText somePopup)
	{
		...
		for (int i = 0; i < num2; i++)
		{
			...
			if (dust.scale >= 0.9f)
			{
				...
				dust2.color = new Color((byte)255, (byte)255, (byte)255, (byte)255);
				dust2.scale *= 0.65f;
				dust2.fadeIn = 1.1f;
			}
		}
		for (int j = 0; j < num3; j++)
		{
			...
			if (dust3.scale >= 0.9f)
			{
				...
				dust4.color = new Color((byte)255, (byte)255, (byte)255, (byte)255);
				dust4.scale *= 0.65f;
				dust4.fadeIn = 1.1f;
			}
		}
	...
	public void Update(int whoAmI)
	{
		...
		{
			color = new Color((byte)255, (byte)(Main.masterColor * 200f), (byte)0, Main.mouseTextColor);
		}
		bool flag = false;
		...
		for (int i = 0; i < 20; i++)
		{
			...
			if (popupText.effectStyle == PopupEffectStyle.Metal || popupText.effectStyle == PopupEffectStyle.MagicWeapon || popupText.effectStyle == PopupEffectStyle.RangedWeapon || popupText.effectStyle == PopupEffectStyle.SummonWeapon || popupText.effectStyle == PopupEffectStyle.MeleeWeapon)
			{
				...
				if (num11 > 0f)
				{
					...
					while (vector9.X <= 20f)
					{
						spriteBatch.Draw(value2, vector4 + vector7 + vector9 * num10, null, new Color((byte)255, (byte)255, (byte)255, (byte)0) * 0.5f * num12, (float)Math.PI / 2f, origin, vector6 * 0.5f, SpriteEffects.None, 0f);
						vector9.X += 20f;
					}
				}
				...
				spriteBatch.Draw(value2, vector4 + vector10, null, new Color((byte)255, (byte)255, (byte)255, (byte)0) * num14, 0f + num13, origin, new Vector2(0.5f, 0.5f) * num10 * 1.3f, SpriteEffects.None, 0f);
				spriteBatch.Draw(value2, vector4 + vector10, null, new Color((byte)255, (byte)255, (byte)255, (byte)0) * num14, (float)Math.PI / 2f + num13, origin, new Vector2(0.5f, 0.5f) * num10, SpriteEffects.None, 0f);
				num13 = 0f;
				spriteBatch.Draw(value2, vector4 + vector10, null, popupText.color * num14, 0f + num13, origin, num10, SpriteEffects.None, 0f);
				spriteBatch.Draw(value2, vector4 + vector10, null, popupText.color * num14, (float)Math.PI / 2f + num13, origin, num10 * 1.3f, SpriteEffects.None, 0f);
				spriteBatch.Draw(value2, vector4 + vector10, null, new Color((byte)255, (byte)255, (byte)255, (byte)0) * num14, 0f + num13, origin, new Vector2(0.5f, 0.5f) * num10 * 1.3f, SpriteEffects.None, 0f);
			...
			if (popupText.effectStyle == PopupEffectStyle.RangedWeapon)
			{
				...
				for (int l = 0; l < text.Length; l++)
				{
					...
					array2[l] = Color.Lerp(new Color((byte)0, (byte)0, (byte)0, (byte)1), popupText.color, num20);
