...
public class MessageBuffer
{
	...
	public void GetData(int start, int length, out int messageType)
	{
		...
		case 13:
		{
			...
			player19.netCameraTarget = (bitsByte19[5] ? new Vector2?(reader.ReadVector2()) : null);
		...
		case 17:
		{
			...
			if (b8 == 0)
			{
				WorldGen.KillTile(num145, num146, flag12);
				if (Main.netMode == 1 && !flag12)
				{
			...
			if (b8 == 2)
			{
				WorldGen.KillWall(num145, num146, flag12);
			}
			if (b8 == 3)
			{
			...
			if (b8 == 4)
			{
				WorldGen.KillTile(num145, num146, flag12, effectOnly: false, noItem: true);
			}
			if (b8 == 5)
			{
		...
		case 148:
		{
			...
			bool flag16 = bitsByte15[0];
			bool flag17 = bitsByte15[1];
		...
		case 23:
		{
			...
			bool flag20 = bitsByte21[3] || !nPC5.active;
			int num220 = -1;
			if (flag20 || nPC5.netID != num218)
			{
				if (flag20)
				{
					nPC5.ResetForNewNPC();
			...
			if (!flag20 && Vector2.DistanceSquared(nPC5.position, vector6) <= (float)(Main.multiplayerNPCSmoothingRange * Main.multiplayerNPCSmoothingRange))
			{
		...
		case 157:
		{
			...
			Color color2 = Main.teamColor[num159];
			if (Main.netMode != 2)
			{
				break;
			}
			...
			for (int num160 = 0; num160 < 255; num160++)
			{
				...
				{
					ChatHelper.SendChatMessageToClient(NetworkText.FromKey(localizedText.Key, player14.name), color2, num160);
				}
			}
		...
		case 65:
		{
			...
			case 0:
				...
				if (Main.netMode == 2)
				{
					NetMessage.TrySendData(65, -1, whoAmI, null, 0, num134, vector3.X, vector3.Y, num135, flag10.ToInt(), num137);
				}
			...
			case 2:
			{
				...
				NetMessage.TrySendData(65, -1, -1, null, 0, num134, vector3.X, vector3.Y, num135, flag10.ToInt(), num137);
				int num138 = -1;
				float num139 = 9999f;
		...
		case 79:
		{
			...
			WorldGen.PlaceObject(x13, y13, type18, mute: false, style3, num204, random, direction);
			if (Main.netMode == 2)
			{
				NetMessage.SendObjectPlacement(whoAmI, x13, y13, type18, style3, num204, random, direction);
			}
			break;
		}
		case 80:
		...
		case 124:
		{
			...
			result4.ReadItem(num207, reader, flag19);
			if (Main.netMode == 2)
			{
				NetMessage.TrySendData(b, -1, num205, null, num205, num206, num207, flag19.ToInt());
			}
			break;
		}
		case 125:
