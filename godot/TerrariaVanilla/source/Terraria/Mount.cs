...
public class Mount
{
	...
	public void UpdateEffects(Player mountedPlayer)
	{
		...
		case 23:
		{
			...
			if (Main.rand.Next(45) == 0)
			{
				...
				Dust dust3 = Dust.NewDustPerfect(vector11 + vector12, 43, Vector2.Zero, 254, new Color((byte)255, (byte)255, (byte)0, (byte)255), 0.3f);
			...
			for (float num15 = 0f; num15 < 1f; num15 += 0.125f)
			{
				if (Main.rand.Next(15) == 0)
				{
					...
					Dust dust4 = Dust.NewDustPerfect(pos4 + spinningpoint, 43, Vector2.Zero, 254, new Color((byte)255, (byte)255, (byte)0, (byte)255), 0.3f);
	...
	public void Draw(List<DrawData> playerDrawData, int drawType, Player drawPlayer, Vector2 Position, Color drawColor, SpriteEffects playerEffect, float shadow)
	{
		...
		case 7:
			if (drawType == 3)
			{
				drawColor = new Color((byte)250, (byte)250, (byte)250, (byte)255) * drawPlayer.stealth * (1f - shadow);
			}
			break;
		case 61:
		...
		case 45:
			if (drawType == 2)
			{
				color = new Color((byte)150, (byte)110, (byte)110, (byte)100);
			}
			break;
		case 11:
			if (drawType == 2)
		...
		case 45:
		{
			...
			{
				color = new Color((byte)255, (byte)220, (byte)220, (byte)200);
				scale = 1.1f;
			}
			for (int k = 0; k < 2; k++)
			{
		...
		case 23:
			if (drawType == 0)
			{
				...
				DrawData item = new DrawData(texture2D, Utils.Floor(vector7), value2, drawColor, num9, origin, scale, spriteEffects);
				item.shader = currentShader;
				playerDrawData.Add(item);
				...
				Color color4 = new Color((byte)80, (byte)70, (byte)40, (byte)0) * (num13 / 8f + 0.5f) * 0.8f;
				value2.X += width;
				for (int l = 0; l < 4; l++)
				{
					item = new DrawData(texture2D, Utils.Floor(vector7 + ((float)l * ((float)Math.PI / 2f)).ToRotationVector2() * num13), value2, color4, num9, origin, scale, spriteEffects);
					item.shader = currentShader;
					playerDrawData.Add(item);
				}
			}
	...
	private void DoSpawnDust(Player mountedPlayer, bool isDismounting)
	{
		...
		if (_type == 52)
		{
			...
			{
				_ = _data.spawnDust;
				Dust dust = Dust.NewDustDirect(new Vector2(mountedPlayer.position.X - 20f, mountedPlayer.position.Y), mountedPlayer.width + 40, mountedPlayer.height, 267, 0f, 0f, 60, new Color((byte)130, (byte)60, (byte)255, (byte)70));
		...
		if (_type == 62 || _type == 63)
		{
			...
			{
				_ = _data.spawnDust;
				Dust dust3 = Dust.NewDustDirect(new Vector2(mountedPlayer.position.X - 20f, mountedPlayer.position.Y), mountedPlayer.width + 40, mountedPlayer.height, 306, 0f, 0f, 60, new Color((byte)100, (byte)227, (byte)255, (byte)127), 2f);
