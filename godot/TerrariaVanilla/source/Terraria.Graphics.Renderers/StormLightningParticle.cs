...
public class StormLightningParticle : IPooledParticle, IParticle
{
	...
	public void Update(ref ParticleRendererSettings settings)
	{
		...
		for (int i = 0; i < 3; i++)
		{
			...
			{
				Dust dust = Dust.NewDustDirect(StartPosition, 16, 16, 306, 0f, 0f, 0, new Color(color.R, color.G, color.B, (byte)0));
				...
				dust2.color = new Color((byte)255, (byte)255, (byte)255, (byte)0);
				dust2.scale = 1.3f;
			}
		}
		for (int j = -1; j <= 1; j += 2)
		{
			if (Main.rand.Next(4) == 0 && !(Main.rand.NextFloat() > num2 * 0.2f))
			{
				...
				dust3.color = new Color(color.R, color.G, color.B, (byte)0);
				dust3.scale = 1.8f;
				dust3.fadeIn = 0f;
				dust3.noGravity = Main.rand.Next(3) != 0;
				dust3.noLight = (dust3.noLightEmittance = true);
				Dust dust4 = Dust.CloneDust(dust3);
				dust4.color = new Color((byte)255, (byte)255, (byte)255, (byte)0);
				dust4.scale = 1.3f;
			}
		}
		for (int k = 0; k < 2; k++)
