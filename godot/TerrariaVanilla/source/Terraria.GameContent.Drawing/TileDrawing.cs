...
public class TileDrawing : TileDrawingBase
{
	...
	private Color _martianGlow = new Color((byte)0, (byte)0, (byte)0, (byte)0);

	private Color _meteorGlow = new Color((byte)100, (byte)100, (byte)100, (byte)0);

	private Color _lavaMossGlow = new Color((byte)150, (byte)100, (byte)50, (byte)0);

	private Color _kryptonMossGlow = new Color((byte)0, (byte)200, (byte)0, (byte)0);

	private Color _xenonMossGlow = new Color((byte)0, (byte)180, (byte)250, (byte)0);

	private Color _argonMossGlow = new Color((byte)225, (byte)0, (byte)125, (byte)0);

	private Color _violetMossGlow = new Color((byte)150, (byte)0, (byte)250, (byte)0);
	...
	public void Draw(bool solidLayer, bool intoRenderTargets, int waterStyleOverride = -1)
	{
		...
		_martianGlow = new Color(b, b, b, (byte)0);
	...
	private static float DistToWanderingCircle(Point pos, int gridSize, float wanderDist, float cyclesPerTick, uint seed)
	{
		...
		uint num2 = (Hash((uint)point.X) ^ Hash2((uint)point.Y) ^ seed) & 0xFFFFFFu;
		uint num3 = (Hash2((uint)point.X) ^ Hash((uint)point.Y) ^ seed) & 0xFFFFFFu;
	...
	private void DrawSingleTile(Vector2 screenPosition, Vector2 screenOffset, int tileX, int tileY)
	{
		...
		case 692:
		{
			Color color = new Color(Main.DiscoR, Main.DiscoG, Main.DiscoB, (byte)255);
		...
		case 129:
		{
			tileDrawInfo.finalColor = new Color((byte)255, (byte)255, (byte)255, (byte)100);
			int num6 = 2;
		...
		if (Main.tileGlowMask[tileDrawInfo.tileCache.type] != -1)
		{
			...
			{
			case 718:
				color2 = new Color((byte)0, (byte)0, (byte)0, (byte)0);
				break;
			case 717:
			{
			...
			case 390:
				...
				break;
			case 391:
				color2 = new Color((byte)250, (byte)250, (byte)250, (byte)200);
				break;
			case 209:
			...
			case 129:
			{
				...
				color2 = new Color((byte)255, (byte)255, (byte)255, (byte)100);
				break;
			}
			case 725:
			{
				...
				if (opacity > 0f && tileDrawInfo.tileFrameX % 36 == 0 && tileDrawInfo.tileFrameY == 54)
				{
					...
					color2 = new Color((byte)255, (byte)255, (byte)255, (byte)100) * opacity;
	...
	public Texture2D GetTileDrawTexture(Tile tile, int tileX, int tileY)
	{
		...
		tileVariationkey.TileType = tile.type;
		tileVariationkey.TileStyle = 0;
		tileVariationkey.PaintColor = tile.color();
		TilePaintSystemV2.TileVariationkey key = tileVariationkey;
		switch (tile.type)
		{
		case 5:
				...
				for (float num11 = 0f; num11 < 1f; num11 += 1f / 3f)
				{
					...
					for (int j = 0; j < 2; j++)
					{
						...
						for (int k = 0; k < 2; k++)
						{
							...
							Main.tileBatch.Draw(drawData.drawTexture, Utils.Floor(normalTilePosition + new Vector2(j * 16, k * 16) + ((num12 + num11) * ((float)Math.PI * 2f)).ToRotationVector2() * 4f), new Rectangle(rectangle2.X + j * 18, rectangle2.Y + k * 18, rectangle2.Width, rectangle2.Height), color, _zero, 1f, drawData.tileSpriteEffect);
						}
					}
				}
			}
	...
	private Color GetFallenStarFurnitureFlameColor()
	{
		...
		return new Color(150, num2, num2, (byte)50);
	}
	...
	private Color GetHallowedFurnitureFlameColor()
	{
		...
		return new Color(170, num2, num2, (byte)75);
	}

	private Color GetCloudFurnitureFlameColor()
	{
		return GetWrappedFurnitureFlameColor(new Color((byte)255, (byte)255, (byte)255, (byte)0));
	}

	private Color GetLibrarianFurnitureFlameColor()
	{
		return GetWrappedFurnitureFlameColor(new Color((byte)255, (byte)255, (byte)255, (byte)0), 0.25f);
	}
	...
	private TileFlameData GetTileFlameData(int tileX, int tileY, int type, int tileFrameY)
	{
		...
		case 270:
		{
			...
			tileFlameData.flameColor = new Color((byte)200, (byte)200, (byte)200, (byte)0);
		...
		case 581:
		{
			...
			tileFlameData.flameColor = new Color((byte)200, (byte)100, (byte)100, (byte)0);
			tileFlameData.flameCount = 1;
			return tileFlameData;
		}
		default:
		{
			...
			case 7:
				result.flameCount = 4;
				result.flameColor = new Color((byte)50, (byte)50, (byte)50, (byte)0);
			...
			case 1:
				...
				case 14:
					result.flameCount = 8;
					result.flameColor = new Color((byte)75, (byte)75, (byte)75, (byte)0);
					result.flameRangeXMin = -10;
					result.flameRangeXMax = 11;
					result.flameRangeYMin = -10;
					result.flameRangeYMax = 11;
				...
				case 44:
					result.flameCount = 3;
					result.flameColor = new Color((byte)200, (byte)200, (byte)200, (byte)150);
					result.flameRangeXMin = -10;
					result.flameRangeXMax = 11;
					result.flameRangeYMin = -10;
					result.flameRangeYMax = 1;
				...
				default:
					result.flameCount = 7;
					result.flameColor = new Color((byte)100, (byte)100, (byte)100, (byte)0);
					result.flameRangeXMin = -10;
					result.flameRangeXMax = 11;
					result.flameRangeYMin = -10;
					result.flameRangeYMax = 1;
	...
	private void DrawSingleTile_Flames(Vector2 screenPosition, Vector2 screenOffset, int tileX, int tileY, TileDrawInfo drawData)
	{
		...
		if (drawData.typeCache == 215 && drawData.tileFrameY < 36)
		{
			...
			case 5:
				...
				break;
			case 14:
				color = new Color((byte)50, (byte)50, (byte)100, (byte)20);
				break;
			case 15:
				color = new Color((byte)255, (byte)255, (byte)255, (byte)200);
				break;
			}
		...
		if (drawData.typeCache == 356 && Main.sundialCooldown == 0)
		{
			...
			Color color3 = new Color((byte)100, (byte)100, (byte)100, (byte)0);
		...
		else if (Main.tileFlame[drawData.typeCache])
		{
			...
			default:
			{
				Color color4 = new Color((byte)100, (byte)100, (byte)100, (byte)0);
				if (drawData.tileCache.type == 4)
				{
					...
					case 14:
						...
						break;
					case 22:
						color4 = new Color((byte)50, (byte)50, (byte)100, (byte)20);
						break;
					case 23:
						color4 = new Color((byte)255, (byte)255, (byte)255, (byte)200);
						break;
					}
				}
				if (drawData.tileCache.type == 646)
				{
					color4 = new Color((byte)100, (byte)100, (byte)100, (byte)150);
				}
				for (int k = 0; k < 7; k++)
				{
		...
		if (num136 != 2)
		{
			...
			Color color5 = new Color(Main.mouseTextColor, Main.mouseTextColor, Main.mouseTextColor, (byte)0);
			if (num136 == 0)
			{
				color5 *= 0.75f;
			}
	...
	private void DrawXmasTree(Vector2 screenPosition, Vector2 screenOffset, int tileX, int tileY, TileDrawInfo drawData)
	{
		...
		if (num2 > 0)
		{
			...
			if (num2 != 3)
			{
				color2 = new Color((byte)255, (byte)255, (byte)255, (byte)255);
			}
		...
		{
			num5--;
			Main.tileBatch.Draw(TextureAssets.XmasTree[4].Value, new Vector2((float)(tileX * 16 - (int)screenPosition.X) - ((float)drawData.tileWidth - 16f) / 2f, tileY * 16 - (int)screenPosition.Y + drawData.tileTop + num) + screenOffset, new Rectangle(66 * num5, 130 * Main.tileFrame[171], 64, 128), new Color((byte)255, (byte)255, (byte)255, (byte)255), _zero, 1f, SpriteEffects.None);
		}
	}
	...
	private void GetTileDrawData(int x, int y, Tile tileCache, ushort typeCache, ref short tileFrameX, ref short tileFrameY, out int tileWidth, out int tileHeight, out int tileTop, out int halfBrickHeight, out int addFrX, out int addFrY, out SpriteEffects tileSpriteEffect, out Texture2D glowTexture, out Rectangle glowSourceRect, out Color glowColor)
	{
		...
		case 412:
			...
			glowColor = new Color((byte)255, (byte)255, (byte)255, (byte)255);
			break;
		case 657:
			if (tileFrameY >= 54)
			{
		...
		case 701:
			...
			glowSourceRect = new Rectangle(tileFrameX, tileFrameY + addFrY, tileWidth, tileHeight);
			glowColor = new Color((byte)255, (byte)255, (byte)255, (byte)0) * ((float)(int)Main.mouseTextColor / 255f);
			break;
		case 634:
		...
		case 580:
			...
			glowSourceRect = new Rectangle(tileFrameX, tileFrameY + addFrY, tileWidth, tileHeight);
			glowColor = new Color((byte)225, (byte)110, (byte)110, (byte)0);
			break;
		case 564:
			if (tileCache.frameX < 36)
			{
				...
				glowSourceRect = new Rectangle(tileFrameX, tileFrameY + addFrY, tileWidth, tileHeight);
				glowColor = new Color((byte)200, (byte)200, (byte)200, (byte)0) * ((float)(int)Main.mouseTextColor / 255f);
			}
			addFrY = 0;
			break;
		case 184:
		...
		case 463:
			...
			glowSourceRect = new Rectangle(tileFrameX, tileFrameY + addFrY, tileWidth, tileHeight);
			glowColor = new Color((byte)127, (byte)127, (byte)127, (byte)0);
			break;
		case 19:
		{
	...
	private void DrawTiles_EmitParticles(int j, int i, Tile tileCache, ushort typeCache, short tileFrameX, short tileFrameY, Color tileLight)
	{
		...
		if (typeCache == 718 && !Main.dayTime && _rand.Next(3) == 0 && !WorldGen.SolidTile3(i, j - 1))
		{
			...
			if (Main.player[Main.myPlayer].RollLuck(30) == 0)
			{
				...
				Main.dust[num5].color = new Color((byte)255, (byte)255, (byte)255, (byte)0);
		...
		else if (Main.tileShine[typeCache] > 0)
		{
			...
			case 178:
			{
				...
				{
				case 0:
					newColor = new Color((byte)255, (byte)0, (byte)255, (byte)255);
					break;
				case 1:
					newColor = new Color((byte)255, (byte)255, (byte)0, (byte)255);
					break;
				case 2:
					newColor = new Color((byte)0, (byte)0, (byte)255, (byte)255);
					break;
				case 3:
					newColor = new Color((byte)0, (byte)255, (byte)0, (byte)255);
					break;
				case 4:
					newColor = new Color((byte)255, (byte)0, (byte)0, (byte)255);
					break;
				case 5:
					newColor = new Color((byte)255, (byte)255, (byte)255, (byte)255);
					break;
				case 6:
					newColor = new Color((byte)255, (byte)255, (byte)0, (byte)255);
					break;
			...
			if (typeCache == 211)
			{
				newColor = new Color((byte)50, (byte)255, (byte)100, (byte)255);
			}
	...
	private void DrawSpecialTilesLegacy(Vector2 screenPosition, Vector2 offSet)
	{
		...
		for (int i = 0; i < _specialTilesCount; i++)
		{
			...
			if (type == 237)
			{
				Main.tileBatch.Draw(TextureAssets.SunOrb.Value, new Vector2((float)(num * 16 - (int)screenPosition.X) + 8f, num2 * 16 - (int)screenPosition.Y - 36) + offSet, new Rectangle(0, 0, TextureAssets.SunOrb.Width(), TextureAssets.SunOrb.Height()), new Color(Main.mouseTextColor, Main.mouseTextColor, Main.mouseTextColor, (byte)0), new Vector2(TextureAssets.SunOrb.Width() / 2, TextureAssets.SunOrb.Height() / 2), 1f, SpriteEffects.None);
			}
			...
			if (type == 620)
			{
				...
				Main.tileBatch.Draw(value5, vector, value6, new Color((byte)255, (byte)255, (byte)255, (byte)255), Vector2.Zero, 1f, SpriteEffects.None);
				for (int j = 0; j < num11; j++)
				{
					Color color5 = new Color((byte)127, (byte)127, (byte)127, (byte)0).MultiplyRGBA(Main.hslToRgb((Main.GlobalTimeWrappedHourly + (float)j / (float)num11) % 1f, 1f, 0.5f));
				...
				Main.tileBatch.Draw(value5, vector, value6, new Color((byte)255, (byte)255, (byte)255, (byte)0) * 0.1f, Vector2.Zero, 1f, SpriteEffects.None);
			}
		}
	}
			...
			try
			{
				...
				if (flag2 && frameY >= 198 && frameX >= 22)
				{
					...
					case 22:
					{
						...
						DrawNature(treeTopTexture, vector, new Rectangle(treeFrame * (topTextureFrameWidth3 + 2), 0, topTextureFrameWidth3, topTextureFrameHeight3), color6, num15 * num3, new Vector2(topTextureFrameWidth3 / 2, topTextureFrameHeight3), 1f, SpriteEffects.None, 0f);
						if (type == 634)
						{
							Texture2D value3 = TextureAssets.GlowMask[316].Value;
							Color white3 = Color.White;
							DrawNatureGlowmask(value3, vector, new Rectangle(treeFrame * (topTextureFrameWidth3 + 2), 0, topTextureFrameWidth3, topTextureFrameHeight3), white3, num15 * num3, new Vector2(topTextureFrameWidth3 / 2, topTextureFrameHeight3), 1f, SpriteEffects.None, 0f);
						}
						break;
					}
					case 44:
					{
						...
						Vector2 position2 = Utils.Floor(new Vector2(x * 16, y * 16) - unscaledPosition) + zero + new Vector2(16f, 12f);
						float num12 = 0f;
						if (!flag)
						{
					...
					case 66:
					{
						...
						Vector2 position = Utils.Floor(new Vector2(x * 16, y * 16) - unscaledPosition) + zero + new Vector2(0f, 18f);
						float num8 = 0f;
						if (!flag)
						{
				...
				if (type == 323 && frameX >= 88 && frameX <= 132)
				{
					...
					DrawNature(treeTopTexture2, position3, new Rectangle(num16 * (num17 + 2), y2, num17, num18), color7, num21 * num3, new Vector2(num17 / 2, num18), 1f, SpriteEffects.None, 0f);
				}
			}
			catch
			{
	...
	private void DrawTeleportationPylons()
	{
		...
		for (int i = 0; i < num2; i++)
		{
			...
			Color color = new Color((byte)255, (byte)255, (byte)255, (byte)0) * 0.1f * num6;
	...
	private void EmitAlchemyHerbParticles(int j, int i, int style)
	{
		...
		{
			int num6 = Dust.NewDust(newColor: new Color((byte)50, (byte)255, (byte)255, (byte)255), Position: new Vector2(i * 16, j * 16), Width: 16, Height: 16, Type: 43, SpeedX: 0f, SpeedY: 0f, Alpha: 254, Scale: 0.5f);
