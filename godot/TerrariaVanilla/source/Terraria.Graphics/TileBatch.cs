...
public class TileBatch
{
	...
	private void SwitchBatch(Texture2D texture, uint layerStack)
	{
		...
		uint num = (layerStack >> 14) | (layerStack & 0xFFFFu);
		...
		uint num2 = (currentBatchKey.LayerStack >> 14) | (currentBatchKey.LayerStack & 0xFFFFu);
	...
	private unsafe void FillVertexBuffer(Texture2D texture, SpriteData[] sprites, int offset, int count, int vbSpriteOffset)
	{
		...
		{
			fixed (VertexPositionColorTexture* ptr3 = &_vertices[vbSpriteOffset * 4])
			{
				SpriteData* ptr2 = ptr;
				...
				for (int i = 0; i < count; i++)
				{
					...
					if (ptr2->Rotation != 0f)
					{
						num3 = (float)Math.Cos(ptr2->Rotation);
						num4 = (float)Math.Sin(ptr2->Rotation);
					}
					else
					{
						num3 = 1f;
					...
					float num5 = ptr2->Origin.X / ptr2->Source.Z;
					float num6 = ptr2->Origin.Y / ptr2->Source.W;
					...
					for (int j = 0; j < 4; j++)
					{
						...
						float num9 = (num7 - num5) * ptr2->Destination.Z;
						float num10 = (num8 - num6) * ptr2->Destination.W;
						float x = ptr2->Destination.X + num9 * num3 - num10 * num4;
						float y = ptr2->Destination.Y + num9 * num4 + num10 * num3;
						...
						ptr4->Position.Y = y;
						ptr4->Position.Z = 0f;
						ptr4->TextureCoordinate.X = (ptr2->Source.X + num7 * ptr2->Source.Z) * num;
						ptr4->TextureCoordinate.Y = (ptr2->Source.Y + num8 * ptr2->Source.W) * num2;
						ptr4++;
					}
					ptr2++;
				}
