...
public class SpriteDrawBuffer
{
	...
	{
		spriteBatch.Begin(SpriteSortMode.Deferred, null, null, null, rasterizer, null, transformation);
		spriteBatch.End();
	}
	...
	private void CheckBuffers()
	{
		if (vertexBuffer == null || vertexBuffer.IsDisposed)
		{
			...
			vertexBuffer = new DynamicVertexBuffer(graphicsDevice, typeof(VertexPositionColorTexture), bufferSize * 4, BufferUsage.WriteOnly);
		}
		if (indexBuffer == null || indexBuffer.IsDisposed)
		{
			...
			indexBuffer = new IndexBuffer(graphicsDevice, typeof(ushort), bufferSize * 6, BufferUsage.WriteOnly);
			indexBuffer.SetData(GenIndexBuffer(bufferSize));
		}
	}
	...
	public int DrawRange(int index, int count)
	{
		...
		while (count > 0)
		{
			...
			{
				vertexBuffer.SetData(vertices, index * 4, Math.Min(vertices.Length - index * 4, bufferSize * 4), SetDataOptions.Discard);
				uploadedSpriteIndex = index;
			}
			int num2 = Math.Min(count, bufferSize);
			int num3 = index - uploadedSpriteIndex;
			graphicsDevice.DrawIndexedPrimitives(PrimitiveType.TriangleList, num3 * 4, 0, num2 * 4, 0, num2 * 2);
			count -= num2;
			index += num2;
			num++;
		}
	...
	public void Draw(Texture2D texture, Vector2 position, Rectangle? sourceRectangle, VertexColors colors, float rotation, Vector2 origin, Vector2 scale, SpriteEffects effects)
	{
		...
		Draw(texture, new Vector4(position.X, position.Y, z, w), sourceRectangle, colors, rotation, origin, effects);
	}
	...
	{
		Draw(texture, new Vector4(destination.X, destination.Y, destination.Width, destination.Height), sourceRectangle, colors, rotation, origin, effects);
	}
