...
public static class Minecart
{
	...
	public struct Customization
	{
		...
		public static Customization Default
		{
			get
			{
				Customization result = default(Customization);
				result.MinecartTextureWidth = 50f;
				result.MagnetOffset = new Vector2(25f, 26f);
				result.WheelOffset = new Vector2(12f, 0f);
				return result;
			}
		}
	}
	...
	public static BitsByte TrackCollision(Player Player, ref Vector2 Position, ref Vector2 Velocity, ref Vector2 lastBoost, int Width, int Height, bool followDown, bool followUp, int fallStart, bool trackOnly, Mount.MountDelegatesData delegatesData)
	{
		...
		Vector2 vector = new Vector2((float)(Width / 2) - minecartSettings.MinecartTextureWidth / 2f, Height / 2);
		Vector2 vector2 = Position + new Vector2((float)(Width / 2) - minecartSettings.MinecartTextureWidth / 2f, Height / 2) + minecartSettings.MagnetOffset;
			...
			if (flag)
			{
				...
				case -3:
					if (trackState == TrackState.OnFront)
					{
						...
						vector5 = Vector2.Transform(matrix: (Velocity.X > 0f) ? ((_leftSideConnection[num9] != 2) ? Matrix.CreateRotationZ((float)Math.PI / 4f) : Matrix.CreateRotationZ(-(float)Math.PI / 4f)) : ((_rightSideConnection[num9] != 2) ? Matrix.CreateRotationZ(-(float)Math.PI / 4f) : Matrix.CreateRotationZ((float)Math.PI / 4f)), position: new Vector2(Velocity.X, 0f));
		...
		else if (result[1])
		{
			...
			Position.X = vector4.X - minecartSettings.MagnetOffset.X - vector.X - Velocity.X;
			if (vector3.Y == 0f)
			{
				Velocity.Y = 0f;
			}
	...
	{
		Vector2 vector = Position + new Vector2((float)(Width / 2) - settings.MinecartTextureWidth / 2f, Height / 2) + settings.MagnetOffset;
	...
	{
		new Vector2((float)(Width / 2) - settings.MinecartTextureWidth / 2f, Height / 2);
		return Position + new Vector2((float)(Width / 2) - settings.MinecartTextureWidth / 2f, Height / 2) + settings.MagnetOffset;
	}
