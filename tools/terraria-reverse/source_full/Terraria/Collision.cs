...
public class Collision
{
	...
	public static float TileCollisionInStepsOf16(Vector2 Position, Vector2 normalizedDirection, float amount, int Width, int Height)
	{
		...
		{
			Vector2 vector2 = normalizedDirection * num2;
			Vector2 position = Position + vector2;
			Vector2 vector3 = vector + vector2;
			float num3 = Math.Min(num, amount - num2);
			Vector2 vector4 = normalizedDirection * num3;
			Vector4 vec = SlopeCollision(vector3, vector4, width, Height);
			if (vec.XY() != vector3 || vec.ZW() != vector4)
			{
				float num4 = (vec.XY() - vector3).Length();
				return num2 - num4;
			}
			vec = SlopeCollision(vector3 + vector4, vector4, width, Height);
			if (vec.XY() != vector3 + vector4 || vec.ZW() != vector4)
			{
				float num5 = (vec.XY() - vector3 - vector4).Length();
				return num2 + num3 - num5;
			}
			Vector2 vector5 = TileCollision(position, vector4, Width, Height);
			if (vector5 != vector4)
			{
				return num2 + vector5.Length();
			}
		}
		return amount;
	}
	...
	public static bool TryChangingSizeFromBottomCenter(Rectangle hitbox, int targetWidth, int targetHeight, out Rectangle changedHitbox)
	{
		...
		if (num6 > 0)
		{
			...
			int num9;
			int num10 = (num9 = Math.Min(num6 / 2, Math.Min(num8, num7)));
			int val2 = num6 - num9 - num10;
			num9 += Math.Min(val2, num8 - num9);
			val2 = num6 - num9 - num10;
			num10 += Math.Min(val2, num7 - num10);
			changedHitbox.Offset(-num9, 0);
			changedHitbox.Width += num9 + num10;
		}
		else
		{
	...
	public static void LaserScan(Vector2 samplingPoint, Vector2 directionUnit, float samplingWidth, float maxDistance, float[] samples)
	{
		for (int i = 0; i < samples.Length; i++)
		{
			...
			float num6 = 0f;
			num6 = (HitLine(num2, num3, num4, num5, 0, 0, new List<Point>(), out var col) ? ((col.X != num4 || col.Y != num5) ? (new Vector2(Math.Abs(num2 - col.X), Math.Abs(num3 - col.Y)).Length() * 16f) : maxDistance) : (new Vector2(Math.Abs(num2 - col.X), Math.Abs(num3 - col.Y)).Length() * 16f));
			samples[i] = num6;
		}
	}
