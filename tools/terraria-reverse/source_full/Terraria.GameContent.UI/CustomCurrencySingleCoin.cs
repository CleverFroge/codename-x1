...
public class CustomCurrencySingleCoin : CustomCurrencySystem
{
	...
	public override void DrawSavingsMoney(SpriteBatch sb, string text, float shopx, float shopy, long totalCoins, bool horizontal = false)
	{
		...
		{
			_ = 99;
			Vector2 position = new Vector2(shopx + ChatManager.GetStringSize(FontAssets.MouseText.Value, text, Vector2.One).X + 45f, shopy + 50f - drawState.coinYOffset);
			sb.Draw(value, position, null, Color.White, 0f, value.Size() / 2f, CurrencyDrawScale, SpriteEffects.None, 0f);
			Utils.DrawBorderStringFourWay(sb, FontAssets.ItemStack.Value, totalCoins.ToString(), position.X - 11f, position.Y, Color.White, Color.Black, new Vector2(0.3f), 0.75f * drawState.stackTextScale);
		}
		else
		{
			int num2 = ((totalCoins > 99) ? (-6) : 0);
			sb.Draw(value, new Vector2(shopx + 11f, shopy + 75f - drawState.coinYOffset), null, Color.White, 0f, value.Size() / 2f, CurrencyDrawScale, SpriteEffects.None, 0f);
