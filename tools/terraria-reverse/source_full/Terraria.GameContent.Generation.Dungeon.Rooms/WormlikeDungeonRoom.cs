...
public class WormlikeDungeonRoom : DungeonRoom
{
	...
	public void WormlikeRoom(DungeonData data, int i, int j, bool generating)
	{
		...
		for (int k = 0; k < firstSideIterations; k++)
		{
			...
			if (generating)
			{
				GenerateDungeonSquareRoom(data, Vector2D.op_Implicit(point), brickTileType, brickCrackedTileType, brickWallType, num3, num4);
			}
			if (base.Processed)
			{
				num6++;
		...
		for (int l = 0; l < firstSideIterations; l++)
		{
			...
			if (generating)
			{
				GenerateDungeonSquareRoom(data, Vector2D.op_Implicit(point2), brickTileType, brickCrackedTileType, brickWallType, num3, num4);
			}
			if (base.Processed)
			{
				num6++;
