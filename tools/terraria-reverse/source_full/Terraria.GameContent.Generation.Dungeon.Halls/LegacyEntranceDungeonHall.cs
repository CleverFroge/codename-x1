...
public class LegacyEntranceDungeonHall : LegacyDungeonHall
{
	...
	public override void LegacyHall(DungeonData dungeonData, int i, int j, bool generating = false)
	{
		...
		DungeonRoomSearchSettings dungeonRoomSearchSettings = default(DungeonRoomSearchSettings);
		dungeonRoomSearchSettings.Fluff = num4 / 2 + num2;
		DungeonRoomSearchSettings dungeonRoomSearchSettings2 = dungeonRoomSearchSettings;
		List<DungeonRoom> allRoomsInSpots = DungeonUtils.GetAllRoomsInSpots(dungeonData.dungeonRooms, startPos, endPos, dungeonRoomSearchSettings2);
		Vector2D val4 = val2;
		int num6 = 30;
		int num7 = 10;
		int num8 = 0;
			...
			if (generating)
			{
				if (flag)
				{
					...
					if (num8 <= 0)
					{
						...
						dungeonPlatformData.Position = new Point((int)val.X, (int)val.Y);
						dungeonPlatformData.PlacePotsChance = 0.25;
