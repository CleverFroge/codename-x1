...
public class SmartCursorHelper
{
	...
	private static void Step_EmptyBuckets(SmartCursorUsageInfo providedInfo, ref int focusedX, ref int focusedY)
	{
		...
		if (_targets.Count > 0)
		{
			...
			{
				float num4 = Vector2.Distance(new Vector2(_targets[m].X, _targets[m].Y) * 16f + Vector2.One * 8f, providedInfo.mouse);
	...
	private static void Step_Boulders(SmartCursorUsageInfo providedInfo, ref int focusedX, ref int focusedY)
	{
		...
		if (_targets.Count > 0)
		{
			...
			{
				float num2 = Vector2.Distance(new Vector2(_targets[n].X, _targets[n].Y) * 16f + Vector2.One * 8f, providedInfo.mouse);
				if (num == -1f || num2 < num)
				{
					num = num2;
					point = _targets[n];
	...
	private static void Step_MinecartTracks(SmartCursorUsageInfo providedInfo, ref int focusedX, ref int focusedY)
	{
		if ((providedInfo.item.type == 2340 || providedInfo.item.type == 2739) && focusedX == -1 && focusedY == -1)
		{
			...
			if (_targets.Count > 0)
			{
				...
				for (int k = 0; k < _targets.Count; k++)
				{
					...
					{
						float num4 = Vector2.Distance(new Vector2(_targets[k].X, _targets[k].Y) * 16f + Vector2.One * 8f, providedInfo.mouse);
						if (num3 == -1f || num4 < num3)
						{
							num3 = num4;
							point = _targets[k];
		...
		if (_targets.Count > 0)
		{
			...
			for (int n = 0; n < _targets.Count; n++)
			{
				...
				{
					float num6 = Vector2.Distance(new Vector2(_targets[n].X, _targets[n].Y) * 16f + Vector2.One * 8f, providedInfo.mouse);
	...
	private static void Step_Platforms(SmartCursorUsageInfo providedInfo, ref int focusedX, ref int focusedY)
	{
		...
		if (_targets.Count > 0)
		{
			...
			float num3 = -1f;
			Point point = _targets[0];
			Point point2 = _points[0];
			for (int k = 0; k < _targets.Count; k++)
			{
				Point point3 = _targets[k];
				Point point4 = _points[k];
				Vector2 value = providedInfo.mouse - _targets[k].ToWorldCoordinates();
				float num4 = value.Length();
				float num5 = Vector2.Dot(value, point4.ToVector2());
	...
	private static void Step_MulticolorWrench(SmartCursorUsageInfo providedInfo, ref int focusedX, ref int focusedY)
	{
		...
		if (!num)
		{
			bool flag = (toolMode & WiresUI.Settings.MultiToolMode.Red) != 0;
			bool flag2 = (toolMode & WiresUI.Settings.MultiToolMode.Blue) != 0;
			bool flag3 = (toolMode & WiresUI.Settings.MultiToolMode.Green) != 0;
			bool flag4 = (toolMode & WiresUI.Settings.MultiToolMode.Yellow) != 0;
			bool flag5 = (toolMode & WiresUI.Settings.MultiToolMode.Cutter) != 0;
			for (int i = providedInfo.reachableStartX; i <= providedInfo.reachableEndX; i++)
			{
				for (int j = providedInfo.reachableStartY; j <= providedInfo.reachableEndY; j++)
				{
	...
	private static void Step_Acorns(SmartCursorUsageInfo providedInfo, ref int focusedX, ref int focusedY)
	{
		...
		if (_targets.Count > 0)
		{
			...
			for (int n = 0; n < _targets.Count; n++)
			{
				float num5 = Vector2.Distance(new Vector2(_targets[n].X, _targets[n].Y) * 16f + Vector2.One * 8f, providedInfo.mouse);
	...
	public static void Step_Pickaxe_MineSolids(Player player, Vector2 position, Vector2 Center, int width, int direction, SmartCursorUsageInfo providedInfo, List<Point> grappleTargets, ref int focusedX, ref int focusedY)
	{
		...
		if (_targets.Count > 0)
		{
			...
			{
				float num19 = Vector2.Distance(new Vector2(_targets[num18].X, _targets[num18].Y) * 16f + Vector2.One * 8f, value);
				if (num12 == -1f || num19 < num12)
				{
					num12 = num19;
					point = _targets[num18];
				}
			}
			if (Collision.InTileBounds(point.X, point.Y, providedInfo.reachableStartX, providedInfo.reachableStartY, providedInfo.reachableEndX, providedInfo.reachableEndY))
			{
