...
public static class EmergencyStacking
{
	...
	private static void ProcessPendingTransfers(out int freeSlot)
	{
		...
		for (int i = 0; i < PendingTransfers.Count; i++)
		{
			...
			Transfer t2 = PendingTransfers[i];
			DoTransfer(t2);
			if (t2.src.IsAir)
			{
				freeSlot = Math.Min(freeSlot, t2.src.whoAmI);
			}
		}
		if (Main.netMode != 2)
		{
		...
		PendingTransfers.RemoveAll((Transfer t) => t.NumToTransfer == 0);
	...
	private static void FindBestTransfers(StackableItem[] stackableItems, List<Transfer> transfers, int limit)
	{
		for (int i = 0; i < stackableItems.Length; i++)
		{
			...
			transfer.distanceOrder = int.MaxValue;
			...
			for (int j = 0; j < stackableItems.Length; j++)
			{
				...
				{
					Group group = GroupLookup[stackableItem.type];
					transfer = default(Transfer);
					transfer.src = stackableItem.item;
					transfer.dst = other.item;
					transfer.distanceOrder = num / group.DistanceStepSize;
					transfer.preservationOrder = group.StackingPriority;
					transfer.distance = num;
					Transfer transfer3 = transfer;
					if (stackableItem.isOnScreen)
					{
						transfer3.distanceOrder += OnScreenDistancePriorityPenalty;
					}
					if (transfer3.CompareTo(transfer2) < 0)
					{
						transfer2 = transfer3;
					}
				}
			}
			if (transfer2.src != null)
			{
				AddToOrderedList(transfers, limit, transfer2);
			}
		}
	}
