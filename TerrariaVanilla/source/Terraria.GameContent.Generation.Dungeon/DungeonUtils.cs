	...
	public static void CalculatePlatformsAndDoorsOnEdgesOfRoom(DungeonData dungeonData, DungeonBounds innerBounds, DungeonGenerationStyleData styleData, int? doorFluff = null, int? platformFluff = null)
	{
		...
		for (int i = num; i <= num2; i++)
		{
			if (!flag2 && !Main.tile[i, num3 - 1].active())
			{
				...
				dungeonPlatformData.Position = new Point(i, num3 - 1);
				dungeonPlatformData.InAHallway = false;
				dungeonPlatformData.OverrideStyle = styleData.GetPlatformStyle(genRand);
			...
			{
				DungeonPlatformData dungeonPlatformData = default(DungeonPlatformData);
				dungeonPlatformData.Position = new Point(i, num4 + 1);
				dungeonPlatformData.InAHallway = false;
				dungeonPlatformData.OverrideStyle = styleData.GetPlatformStyle(genRand);
				DungeonPlatformData item2 = dungeonPlatformData;
				if (platformFluff.HasValue)
				{
		...
		for (int j = num3; j <= num4; j++)
		{
			if (!flag4 && !Main.tile[num - 1, j].active())
			{
				...
				DungeonDoorData dungeonDoorData = default(DungeonDoorData);
				dungeonDoorData.OverrideBrickTileType = brickTileType;
				dungeonDoorData.OverrideBrickWallType = brickWallType;
				dungeonDoorData.Position = new Point(num - 1, j);
				dungeonDoorData.Direction = -1;
				dungeonDoorData.InAHallway = false;
				dungeonDoorData.SkipOtherDoorsCheck = flag6;
				dungeonDoorData.SkipSpaceCheck = flag6;
				dungeonDoorData.AlwaysClearArea = true;
				DungeonDoorData item3 = dungeonDoorData;
			...
			if (!flag5 && !Main.tile[num2 + 1, j].active())
			{
				...
				dungeonDoorData.OverrideBrickTileType = brickTileType;
				dungeonDoorData.OverrideBrickWallType = brickWallType;
				dungeonDoorData.Position = new Point(num2 + 1, j);
				dungeonDoorData.Direction = 1;
				dungeonDoorData.InAHallway = false;
				dungeonDoorData.SkipOtherDoorsCheck = flag7;
				dungeonDoorData.SkipSpaceCheck = flag7;
				dungeonDoorData.AlwaysClearArea = true;
				DungeonDoorData item4 = dungeonDoorData;
				if (doorFluff.HasValue)
				{
	...
	public static void CalculatePlatformAndDoorsOnHallway(DungeonData dungeonData, Vector2D hallwayPoint, double hallwayDirectionY, DungeonGenerationStyleData styleData, double doorVariance = 0.1)
	{
		...
		if (Math.Abs(hallwayDirectionY) <= doorVariance)
		{
			if (styleData.DoorItemTypes != null)
			{
				...
				dungeonDoorData.OverrideBrickTileType = brickTileType;
				dungeonDoorData.OverrideBrickWallType = brickWallType;
				dungeonDoorData.Position = hallwayPoint.ToPoint();
				dungeonDoorData.Direction = 0;
				dungeonDoorData.InAHallway = true;
				dungeonDoorData.AlwaysClearArea = true;
				DungeonDoorData item = dungeonDoorData;
				if (num >= 0)
				{
		...
		{
			DungeonPlatformData dungeonPlatformData = default(DungeonPlatformData);
			dungeonPlatformData.Position = hallwayPoint.ToPoint();
			dungeonPlatformData.InAHallway = true;
			dungeonPlatformData.OverrideStyle = styleData.GetPlatformStyle(genRand);
			DungeonPlatformData item2 = dungeonPlatformData;
			dungeonData.dungeonPlatformData.Add(item2);
		}
	...
	public static void GenerateFloatingRocksInArea(DungeonData data, int x, int y, int width, int height, ushort tileType, bool includePlatform = true, int platformStyle = -1, int paint = -1, int platformDistance = 7)
	{
		...
		for (; i < num; i++)
		{
			...
			if (includePlatform)
			{
				...
				DungeonPlatformData dungeonPlatformData = default(DungeonPlatformData);
				dungeonPlatformData.Position = new Point(x2, num6);
				dungeonPlatformData.InAHallway = false;
				dungeonPlatformData.OverrideHeightFluff = 0;
				dungeonPlatformData.ForcePlacement = true;
				dungeonPlatformData.OverrideMaxLengthAllowed = 5 + num8;
				dungeonPlatformData.canPlaceHereCallback = (DungeonData dungeonData, int platformX, int platformY) => platformX >= x && platformX <= x + width && platformY >= y && platformY <= y + height;
				DungeonPlatformData item = dungeonPlatformData;
				if (platformStyle > -1)
				{
