...
public class WaterShaderData : ScreenShaderData
{
	...
	public override void Apply()
	{
		if (_usingRenderTargets && Main.IsGraphicsDeviceAvailable)
		{
			...
			UseTargetPosition(unscaledScreenPosition + vector - Main.waterTarget.Position);
			UseImageOffset(-(vector2 - _lastDistortionDrawOffset / 0.25f));
			base.Apply();
		}
	}
	...
	private void InitRenderTargets(int width, int height)
	{
		...
		try
		{
			_noDistortionTexture = new Texture2D(Main.instance.GraphicsDevice, 1, 1, false, SurfaceFormat.Color);
			_noDistortionTexture.SetData(new Color[1]
			{
