	...
	public static void LoadWorld()
	{
		...
		try
		{
			...
			try
			{
				...
				int num3 = ((num2 <= 0) ? StatusID.UnknownError : ((num2 > 319) ? StatusID.LaterVersion : ((num2 > 87) ? LoadWorld_Version2(binaryReader) : LoadWorld_Version1_Old_BeforeRelease88(binaryReader))));
				if (num2 < 141)
				{
					if (!_isWorldOnCloud)
					{
	...
	private static void _SaveWorld(bool useCloudSaving, bool resetTime, bool useTemps, bool canBeSkipped)
	{
		...
		if (Monitor.TryEnter(IOLock))
		{
			try
			{
				...
				{
					InternalSaveWorld(useCloudSaving, resetTime, useTemps);
				});
				return;
			}
			finally
		...
		if (!canBeSkipped)
		{
			lock (IOLock)
			{
				_SaveWorld(useCloudSaving, resetTime, useTemps, canBeSkipped);
			}
		}
	}
