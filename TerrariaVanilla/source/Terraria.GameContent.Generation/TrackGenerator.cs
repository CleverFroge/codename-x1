...
public class TrackGenerator
{
	...
	private static readonly ushort[] InvalidTiles = new ushort[37]
	{
	...
	private void PlacePath()
	{
		...
		for (int i = 0; i < _length; i++)
		{
			...
			{
				TrackHistory trackHistory = _history[i];
				if (Main.tile[trackHistory.X, trackHistory.Y - j - 1].wall == 244)
				{
					Main.tile[trackHistory.X, trackHistory.Y - j - 1].wall = 0;
				}
				if (Main.tile[trackHistory.X, trackHistory.Y - j].wall == 244)
				{
					Main.tile[trackHistory.X, trackHistory.Y - j].wall = 0;
				}
				if (Main.tile[trackHistory.X, trackHistory.Y - j + 1].wall == 244)
				{
					Main.tile[trackHistory.X, trackHistory.Y - j + 1].wall = 0;
				}
				if (Main.tile[trackHistory.X, trackHistory.Y - j].active() && Main.tile[trackHistory.X, trackHistory.Y - j].type == 135)
				{
					array[i] = true;
				}
				if (Main.tile[trackHistory.X, trackHistory.Y - j].type != 379)
				{
					WorldGen.KillTile(trackHistory.X, trackHistory.Y - j, fail: false, effectOnly: false, noItem: true);
				}
			}
		}
		for (int k = 0; k < _length; k++)
		{
			...
			if (!Main.tile[trackHistory2.X, trackHistory2.Y - playerHeight].active() || Main.tile[trackHistory2.X, trackHistory2.Y - playerHeight].type != 379)
			{
				Tile.SmoothSlope(trackHistory2.X, trackHistory2.Y - playerHeight);
			}
			bool wire = Main.tile[trackHistory2.X, trackHistory2.Y].wire();
			bool wire2 = Main.tile[trackHistory2.X, trackHistory2.Y].wire2();
			bool wire3 = Main.tile[trackHistory2.X, trackHistory2.Y].wire3();
			bool wire4 = Main.tile[trackHistory2.X, trackHistory2.Y].wire4();
			if (array[k] && k < _length && k > 0 && _history[k - 1].Y == trackHistory2.Y && _history[k + 1].Y == trackHistory2.Y)
			{
				Main.tile[trackHistory2.X, trackHistory2.Y].ClearEverything();
				WorldGen.PlaceTile(trackHistory2.X, trackHistory2.Y, 314, mute: false, forced: true, -1, 1);
			}
			else
			{
			...
			Main.tile[trackHistory2.X, trackHistory2.Y].wire2(wire2);
			Main.tile[trackHistory2.X, trackHistory2.Y].wire3(wire3);
			Main.tile[trackHistory2.X, trackHistory2.Y].wire4(wire4);
			if (k == 0)
			{
				continue;
			}
	...
	private static bool IsLocationInvalid(int x, int y)
	{
		...
		if (WorldGen.SecretSeed.dualDungeons.Enabled && DungeonUtils.InAnyPotentialDungeonBounds(x, y))
		{
			return true;
		}
		...
		int num = (Main.tile[x, y].active() ? Main.tile[x, y].type : (-1));
		for (int j = 0; j < InvalidTiles.Length; j++)
		{
			if (num == InvalidTiles[j])
			{
				return true;
			}
		}
