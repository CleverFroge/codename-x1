...
public class ParticleOrchestrator
{
	...
	private static void Spawn_PaladinsShieldHit(ParticleOrchestraSettings settings)
	{
		...
		if (Main.player.IndexInRange(num))
		{
			...
			fadingParticle.ColorTint = new Color((byte)255, (byte)255, (byte)255, (byte)220) * 0.85f;
		...
		if (Main.player.IndexInRange(num2))
		{
			...
			fadingParticle2.ColorTint = new Color((byte)255, (byte)255, (byte)255, (byte)127) * 0.75f;
	...
	private static void Spawn_StormLightningWindup(ParticleOrchestraSettings settings)
	{
		...
		for (float num5 = 0f; num5 < 1f; num5 += 1f / num2)
		{
			...
			randomizedFrameParticle2.ColorTint = new Color((byte)255, (byte)255, (byte)255, (byte)0);
	...
	private static void Spawn_StormLightning(ParticleOrchestraSettings settings)
	{
		...
		{
			int num8 = Main.rand.Next(14, (byte)22);
	...
	public static void SpawnLightningExplosionDust(Vector2 position, Color customColor)
	{
		...
		for (int l = 0; l < 6; l++)
		{
			...
			dust4.color = new Color(customColor.R, customColor.G, customColor.B, (byte)0);
			dust4.scale = 2.8f;
			dust4.fadeIn = 0f;
			dust4.noGravity = Main.rand.Next(3) != 0;
			Dust dust5 = Dust.CloneDust(dust4);
			dust5.color = new Color((byte)255, (byte)255, (byte)255, (byte)0);
			dust5.scale = 1.9f;
		}
		for (int m = 0; m < 6; m++)
		{
			Dust dust6 = Dust.NewDustPerfect(position, 306, new Vector2(0f, -2f).RotatedByRandom(1.5707963705062866));
			dust6.color = new Color(customColor.R, customColor.G, customColor.B, (byte)0);
			dust6.scale = 2.8f;
			dust6.fadeIn = 0f;
			dust6.noGravity = Main.rand.Next(3) != 0;
			Dust dust7 = Dust.CloneDust(dust6);
			dust7.color = new Color((byte)255, (byte)255, (byte)255, (byte)0);
			dust7.scale = 1.9f;
		}
		for (int n = 0; n < 3; n++)
		{
	...
	private static void Spawn_LakeSparkle(ParticleOrchestraSettings settings)
	{
		...
		fadingParticle.ColorTint = new Color((byte)255, (byte)255, (byte)255, (byte)0) * Main.rand.NextFloat();
	...
	{
		ParticleRepelDetails particleRepelDetails = default(ParticleRepelDetails);
		particleRepelDetails.SourcePosition = position;
		particleRepelDetails.Radius = radius;
		particleRepelDetails.IsInWater = wet;
		ParticleRepelDetails details = particleRepelDetails;
	...
	private static void Spawn_RainbowBoulder4(ParticleOrchestraSettings settings, bool pet = false)
	{
		...
		for (float num6 = 0f; num6 < (float)num2; num6 += 1f)
		{
			...
			prettySparkleParticle2.ColorTint = new Color((byte)255, (byte)255, (byte)255, (byte)0);
	...
	private static void Spawn_RainbowBoulder1(ParticleOrchestraSettings settings)
	{
		...
		for (float num4 = 0f; num4 < 1f; num4 += 1f / num2)
		{
			...
			prettySparkleParticle.Velocity = vector2 + zero;
			prettySparkleParticle.AccelerationPerFrame = num5.ToRotationVector2() * -(vector2 / num6) - zero * 1f / num6;
			prettySparkleParticle.ColorTint = new Color((byte)255, (byte)255, (byte)255, (byte)0);
			prettySparkleParticle.LocalPosition = settings.PositionInWorld + vector3;
			prettySparkleParticle.Rotation = rotation;
	...
	private static void Spawn_RainbowBoulder3(ParticleOrchestraSettings settings)
	{
		...
		for (int i = 0; i < 4; i++)
		{
			...
			if (num8 != 6000)
			{
				...
				dust.scale /= 2f;
				dust.fadeIn *= 0.75f;
				dust.color = new Color((byte)255, (byte)255, (byte)255, (byte)255);
				dust.noLight = true;
				dust.noLightEmittance = true;
			}
		}
	...
	private static void Spawn_DeadCellsBarrelExplosion(ParticleOrchestraSettings settings)
	{
		...
		Color colorTint = new Color((byte)52, (byte)208, (byte)254, (byte)20);
		Color color = new Color((byte)52, (byte)208, (byte)254, (byte)20);
		float num = 25f;
		FadingParticle fadingParticle = _poolFading.RequestParticle();
		...
		{
			Dust dust = Dust.NewDustPerfect(settings.PositionInWorld, 267, Vector2.Zero, 0, new Color((byte)100, (byte)200, (byte)255, (byte)127), 2f);
			dust.noGravity = true;
	...
	private static void Spawn_MoonLordWhipEye(ParticleOrchestraSettings settings)
	{
		...
		if (val.IsLoaded)
		{
			...
			fadingParticle.ColorTint = new Color((byte)255, (byte)255, (byte)255, (byte)0);
	...
	private static void Spawn_DeadCellsMushroomBoiExplosion(ParticleOrchestraSettings settings)
	{
		...
		Color lightColorTint = new Color((byte)255, (byte)10, (byte)10, (byte)50) * 0.35f;
		...
		Rectangle value2 = new Rectangle(0, 101, 100, 100);
		Rectangle value3 = new Rectangle(0, 202, 100, 100);
		Color colorTint = new Color((byte)255, (byte)10, (byte)10, (byte)127) * 0.45f;
		Color color = new Color((byte)255, (byte)10, (byte)10, (byte)127) * 0.45f;
		float num2 = 15f;
		FadingParticle fadingParticle = _poolFading.RequestParticle();
		fadingParticle.SetBasicInfo(textureAsset, value, Vector2.Zero, settings.PositionInWorld);
		fadingParticle.SetTypeInfo(num2);
	...
	private static void Spawn_DeadCellsMushroomBoiTargetFound(ParticleOrchestraSettings settings)
	{
		...
		shockIconParticle.ColorTint = new Color((byte)255, (byte)255, (byte)10, (byte)150);
	...
	private static void Spawn_BlackLightningSmall(ParticleOrchestraSettings settings)
	{
		...
		int num4 = 916;
		Main.instance.LoadProjectile(num4);
		Color value = new Color((byte)255, (byte)255, (byte)255, (byte)255);
		...
		for (float num5 = 0f; num5 < 1f; num5 += 1f / num2)
		{
			...
			Color colorTint = new Color((byte)0, (byte)0, (byte)0, (byte)255);
			int num7 = Main.rand.Next(4);
			if (num7 == 1)
			{
				colorTint = Color.Lerp(new Color((byte)106, (byte)90, (byte)205, (byte)127), Color.Black, 0.1f + 0.7f * Main.rand.NextFloat());
			}
			if (num7 == 2)
			{
				colorTint = Color.Lerp(new Color((byte)106, (byte)90, (byte)205, (byte)60), Color.Black, 0.1f + 0.8f * Main.rand.NextFloat());
			}
			RandomizedFrameParticle randomizedFrameParticle = _poolRandomizedFrame.RequestParticle();
			randomizedFrameParticle.SetBasicInfo(TextureAssets.Projectile[num4], null, Vector2.Zero, vector);
	...
	private static void Spawn_BlueLightningSmallLong(ParticleOrchestraSettings settings)
	{
		...
		if (!(num < (float)num3))
		{
			...
			while (num4 < num)
			{
				ParticleOrchestraSettings settings2 = default(ParticleOrchestraSettings);
				settings2.PositionInWorld = positionInWorld;
				settings2.MovementVector = vector * 2f;
				Spawn_BlueLightningSmall(settings2);
				num4 += (float)num2;
	...
	private static void Spawn_BlueLightningSmall(ParticleOrchestraSettings settings)
	{
		...
		short num4 = 916;
		Main.instance.LoadProjectile(num4);
		Color value = new Color((byte)133, (byte)255, (byte)255, (byte)0);
		Color value2 = new Color((byte)15, (byte)100, (byte)155, (byte)0);
