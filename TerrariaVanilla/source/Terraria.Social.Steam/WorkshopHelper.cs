...
public class WorkshopHelper
{
	public class UGCBased
	{
		...
		public class PublishedItemsFinder
		{
			...
			{
				OnSteamUGCQueryCompletedCallResult = CallResult<SteamUGCQueryCompleted_t>.Create((CallResult<SteamUGCQueryCompleted_t>.APIDispatchDelegate)OnSteamUGCQueryCompleted);
				OnSteamUGCRequestUGCDetailsResultCallResult = CallResult<SteamUGCRequestUGCDetailsResult_t>.Create((CallResult<SteamUGCRequestUGCDetailsResult_t>.APIDispatchDelegate)OnSteamUGCRequestUGCDetailsResult);
			}
			...
			public void Refresh()
			{
				...
				m_UGCQueryHandle = SteamUGC.CreateQueryUserUGCRequest(steamID.GetAccountID(), (EUserUGCList)0, (EUGCMatchingUGCType)(-1), (EUserUGCListSortOrder)0, SteamUtils.GetAppID(), SteamUtils.GetAppID(), 1u);
				CoreSocialModule.SetSkipPulsing(shouldSkipPausing: true);
				SteamAPICall_t val = SteamUGC.SendQueryUGCRequest(m_UGCQueryHandle);
				OnSteamUGCQueryCompletedCallResult.Set(val, (CallResult<SteamUGCQueryCompleted_t>.APIDispatchDelegate)OnSteamUGCQueryCompleted);
			...
			private void OnSteamUGCQueryCompleted(SteamUGCQueryCompleted_t pCallback, bool bIOFailure)
			{
				...
				{
					SteamUGC.GetQueryUGCResult(m_UGCQueryHandle, num, out val);
					...
					steamWorkshopItem.Title = val.m_rgchTitle;
					steamWorkshopItem.Description = val.m_rgchDescription;
		...
		public abstract class APublisherInstance
		{
			...
			public void PublishContent(PublishedItemsFinder finder, WorkshopIssueReporter issueReporter, FinishedPublishingAction endAction, string itemTitle, string itemDescription, string contentFolderPath, string previewImagePath, WorkshopItemPublicSettingId publicity, string[] tags)
			{
				...
				_createItemHook = CallResult<CreateItemResult_t>.Create((CallResult<CreateItemResult_t>.APIDispatchDelegate)CreateItemResult);
				_updateItemHook = CallResult<SubmitItemUpdateResult_t>.Create((CallResult<SubmitItemUpdateResult_t>.APIDispatchDelegate)UpdateItemResult);
			...
			private void CreateItem()
			{
				...
				_createItemHook.Set(val, (CallResult<CreateItemResult_t>.APIDispatchDelegate)CreateItemResult);
				CoreSocialModule.SetSkipPulsing(shouldSkipPausing: false);
			}
			...
			private void UpdateItem()
			{
				...
				SteamUGC.SetItemTags(val, (IList<string>)_entryData.Tags, false);
				...
				_updateHandle = val;
				_updateItemHook.Set(val2, (CallResult<SubmitItemUpdateResult_t>.APIDispatchDelegate)UpdateItemResult);
				CoreSocialModule.SetSkipPulsing(shouldSkipPausing: false);
			}
