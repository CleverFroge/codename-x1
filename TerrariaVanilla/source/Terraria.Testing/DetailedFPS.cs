...
public static class DetailedFPS
{
	...
	private struct Frame
	{
		...
		public void Begin(OperationCategory category)
		{
			...
			if (events.Count > 0)
			{
				...
				if (@event.category == OperationCategory.Draw || @event.category == OperationCategory.Update)
				{
	...
	public static float GetCPUUtilization(int numFrames)
	{
		...
		foreach (Frame item in EnumerateFrames())
		{
			...
			Frame.Event @event = item.events[0];
			foreach (Frame.Event event2 in item.events)
			{
	...
	public static bool VsyncAppearsActive()
	{
		...
		foreach (Frame item in EnumerateFrames())
		{
			...
			foreach (Frame.Event event2 in item.events)
			{
				if (@event.category == OperationCategory.Present)
				{
	...
	private static void DrawFrame(int x, Frame frame)
	{
		...
		int val = 0;
		Frame.Event @event = frame.events[0];
		long timestamp = @event.timestamp;
		for (int i = 1; i < frame.events.Count; i++)
		{
			Frame.Event event2 = frame.events[i];
			int num = (int)(Utils.SWTicksToTimeSpan(@event.timestamp - timestamp).TotalMilliseconds * 6.0);
			int num2 = (int)(Utils.SWTicksToTimeSpan(event2.timestamp - timestamp).TotalMilliseconds * 6.0);
			DrawRect(new Rectangle(x, Main.screenHeight - num2, 2, num2 - num), GetColor(@event.category));
			@event = event2;
			val = num2;
		}
