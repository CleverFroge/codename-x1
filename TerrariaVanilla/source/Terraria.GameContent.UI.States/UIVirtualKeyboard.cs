...
public class UIVirtualKeyboard : UIState
{
	...
	public UIVirtualKeyboard(string labelText, string startingText, KeyboardSubmitEvent submitAction, Action cancelAction, int inputMode = 0, bool allowEmpty = false, int maxLength = 20)
	{
		...
		_lastOffsetDown = 0;
		_editingSign = _keyboardContext == 1;
		_editingChest = _keyboardContext == 2;
		...
		float num = 0.5f;
		float num2 = -20f;
		float num3 = -5000 * _editingSign.ToInt();
		num2 = 270f;
		num = 0f;
		float num4 = 516f;
		...
		for (int i = 0; i < 10; i++)
		{
			for (int j = 0; j < 4; j++)
			{
				...
				uITextPanel.OnLeftClick += TypeText;
		...
		_shiftButton.OnLeftClick += delegate
		{
		...
		_symbolButton.OnLeftClick += delegate
		{
			SoundEngine.PlaySound(12);
		...
		_submitButton = new UITextPanel<LocalizedText>((_editingSign || _editingChest) ? Language.GetText("UI.Save") : Language.GetText("UI.Submit"), 0.4f, large: true);
		...
		_submitButton.OnLeftClick += delegate
		{
			Submit();
		};
		...
		_cancelButton.OnLeftClick += delegate
		{
		...
		_submitButton2 = new UITextPanel<LocalizedText>((_editingSign || _editingChest) ? Language.GetText("UI.Save") : Language.GetText("UI.Submit"), 0.72f, large: true);
		...
		_submitButton2.OnLeftClick += delegate
		{
		...
		_cancelButton2.OnLeftClick += delegate
		{
			SoundEngine.PlaySound(11);
			_cancelAction();
		};
		...
		uITextPanel2.OnLeftClick += delegate
		{
			SoundEngine.PlaySound(12);
			_textBox.Backspace();
			ValidateText();
		...
		_label = uIText;
		Append(uIElement);
		_textBox.SetTextMaxLength(maxLength);
		Text = startingText;
		if (Text.Length == 0)
		{
	...
	private void BuildSpaceBarArea(UIPanel mainPanel)
	{
		Action createTheseTwo = delegate
		{
			...
			int x = (flag ? 4 : 5);
			bool editingSign = _editingSign;
			int num = ((flag && editingSign) ? 2 : 3);
			UITextPanel<object> uITextPanel = CreateKeyboardButton(Language.GetText("UI.SpaceButton"), 2, 4, (_editingSign || (_editingChest && flag)) ? num : 6);
			uITextPanel.OnLeftClick += delegate
			{
				PressSpace();
			};
		...
		if (CanRestore())
		{
			...
			restoreBar.OnLeftClick += delegate
			{
				SoundEngine.PlaySound(12);
	...
	private bool TextIsValidForSubmit()
	{
		...
		if (Text.Trim().Length <= 0 && !_editingSign && !_editingChest)
		{
			return _allowEmpty;
		}
		return true;
	...
	protected override void DrawSelf(SpriteBatch spriteBatch)
	{
		...
		if (!Main.editSign && _editingSign)
		{
			IngameFancyUI.Close();
			return;
		}
		if (!Main.editChest && _editingChest)
		{
			IngameFancyUI.Close();
			return;
		}
		bool flag = _textBox.GetOuterDimensions().Width > _textBox.Parent.GetInnerDimensions().Width;
		_textBox.HAlign = (flag ? 1f : 0.5f);
		_textBox.Recalculate();
		...
		string text = Main.GetInputText(Text, _editingSign);
		if (_editingSign && Main.inputTextEnter)
		{
			text += "\n";
		}
		else
		{
			if (_editingChest && Main.inputTextEnter)
			{
			...
			else if (_editingChest && Main.player[Main.myPlayer].chest < 0)
			{
		...
		Color value = new Color(b, b, b, (byte)255);
