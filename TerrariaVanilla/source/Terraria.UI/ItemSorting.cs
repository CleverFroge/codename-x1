...
public class ItemSorting
{
	...
	private class ItemSortingLayers
	{
		...
		{
			List<int> indexesSortable52 = itemsToSort.Where((int i) => inv[i].damage > 0 && !inv[i].consumable && inv[i].ammo == 0 && inv[i].melee && inv[i].pick < 1 && inv[i].hammer < 1 && inv[i].axe < 1).ToList();
			layer.Validate(ref indexesSortable52, inv);
			foreach (int item in indexesSortable52)
			{
				itemsToSort.Remove(item);
			}
			SortIndicesStable(indexesSortable52, delegate(int x, int y)
			{
				int num45 = inv[y].OriginalRarity.CompareTo(inv[x].OriginalRarity);
				if (num45 == 0)
				{
					num45 = inv[y].OriginalDamage.CompareTo(inv[x].OriginalDamage);
				}
				return num45;
			});
			return indexesSortable52;
		});
		...
		{
			List<int> indexesSortable51 = itemsToSort.Where((int i) => (inv[i].damage > 0 && !inv[i].consumable && inv[i].ammo == 0 && inv[i].ranged) || (inv[i].type >= 0 && ItemID.Sets.SortingPriorityWeaponsRanged[inv[i].type] > -1)).ToList();
			layer.Validate(ref indexesSortable51, inv);
			foreach (int item2 in indexesSortable51)
			{
				itemsToSort.Remove(item2);
			}
			SortIndicesStable(indexesSortable51, delegate(int x, int y)
			{
				int num44 = CompareWithPrioritySet(ItemID.Sets.SortingPriorityWeaponsRanged, inv[x].type, inv[y].type);
				if (num44 == 0)
				{
					num44 = inv[y].OriginalRarity.CompareTo(inv[x].OriginalRarity);
				}
				if (num44 == 0)
				{
					num44 = inv[y].OriginalDamage.CompareTo(inv[x].OriginalDamage);
				}
				return num44;
			});
			return indexesSortable51;
		});
		...
		{
			List<int> indexesSortable50 = itemsToSort.Where((int i) => inv[i].damage > 0 && !inv[i].consumable && inv[i].ammo == 0 && inv[i].magic).ToList();
			layer.Validate(ref indexesSortable50, inv);
			foreach (int item3 in indexesSortable50)
			{
				itemsToSort.Remove(item3);
			}
			SortIndicesStable(indexesSortable50, delegate(int x, int y)
			{
				int num43 = inv[y].OriginalRarity.CompareTo(inv[x].OriginalRarity);
				if (num43 == 0)
				{
					num43 = inv[y].OriginalDamage.CompareTo(inv[x].OriginalDamage);
				}
				return num43;
			});
			return indexesSortable50;
		});
		...
		{
			List<int> indexesSortable49 = itemsToSort.Where((int i) => inv[i].damage > 0 && !inv[i].consumable && inv[i].summon).ToList();
			layer.Validate(ref indexesSortable49, inv);
			foreach (int item4 in indexesSortable49)
			{
				itemsToSort.Remove(item4);
			}
			SortIndicesStable(indexesSortable49, delegate(int x, int y)
			{
				int num42 = inv[y].OriginalRarity.CompareTo(inv[x].OriginalRarity);
				if (num42 == 0)
				{
					num42 = inv[y].OriginalDamage.CompareTo(inv[x].OriginalDamage);
				}
				return num42;
			});
			return indexesSortable49;
		});
		...
		{
			List<int> indexesSortable48 = itemsToSort.Where((int i) => inv[i].damage > 0 && inv[i].ammo == 0 && inv[i].pick == 0 && inv[i].axe == 0 && inv[i].hammer == 0).ToList();
			layer.Validate(ref indexesSortable48, inv);
			foreach (int item5 in indexesSortable48)
			{
				itemsToSort.Remove(item5);
			}
			SortIndicesStable(indexesSortable48, delegate(int x, int y)
			{
				int num41 = inv[y].OriginalRarity.CompareTo(inv[x].OriginalRarity);
				if (num41 == 0)
				{
					num41 = inv[y].OriginalDamage.CompareTo(inv[x].OriginalDamage);
				}
				return num41;
			});
			return indexesSortable48;
		});
		...
		{
			List<int> indexesSortable47 = itemsToSort.Where((int i) => inv[i].ammo > 0 && inv[i].damage > 0).ToList();
			layer.Validate(ref indexesSortable47, inv);
			foreach (int item6 in indexesSortable47)
			{
				itemsToSort.Remove(item6);
			}
			SortIndicesStable(indexesSortable47, delegate(int x, int y)
			{
				int num40 = inv[y].OriginalRarity.CompareTo(inv[x].OriginalRarity);
				if (num40 == 0)
				{
					num40 = inv[y].OriginalDamage.CompareTo(inv[x].OriginalDamage);
				}
				return num40;
			});
			return indexesSortable47;
		});
		...
		{
			List<int> indexesSortable46 = itemsToSort.Where((int i) => inv[i].pick > 0 && inv[i].axe > 0).ToList();
			layer.Validate(ref indexesSortable46, inv);
			foreach (int item7 in indexesSortable46)
			{
				itemsToSort.Remove(item7);
			}
			SortIndicesStable(indexesSortable46, (int x, int y) => inv[x].pick.CompareTo(inv[y].pick));
			return indexesSortable46;
		});
		...
		{
			List<int> indexesSortable45 = itemsToSort.Where((int i) => inv[i].hammer > 0 && inv[i].axe > 0).ToList();
			layer.Validate(ref indexesSortable45, inv);
			foreach (int item8 in indexesSortable45)
			{
				itemsToSort.Remove(item8);
			}
			SortIndicesStable(indexesSortable45, (int x, int y) => inv[x].axe.CompareTo(inv[y].axe));
			return indexesSortable45;
		});
		...
		{
			List<int> indexesSortable44 = itemsToSort.Where((int i) => inv[i].pick > 0).ToList();
			layer.Validate(ref indexesSortable44, inv);
			foreach (int item9 in indexesSortable44)
			{
				itemsToSort.Remove(item9);
			}
			SortIndicesStable(indexesSortable44, (int x, int y) => inv[x].pick.CompareTo(inv[y].pick));
			return indexesSortable44;
		});
		...
		{
			List<int> indexesSortable43 = itemsToSort.Where((int i) => inv[i].axe > 0).ToList();
			layer.Validate(ref indexesSortable43, inv);
			foreach (int item10 in indexesSortable43)
			{
				itemsToSort.Remove(item10);
			}
			SortIndicesStable(indexesSortable43, (int x, int y) => inv[x].axe.CompareTo(inv[y].axe));
			return indexesSortable43;
		});
		...
		{
			List<int> indexesSortable42 = itemsToSort.Where((int i) => inv[i].hammer > 0).ToList();
			layer.Validate(ref indexesSortable42, inv);
			foreach (int item11 in indexesSortable42)
			{
				itemsToSort.Remove(item11);
			}
			SortIndicesStable(indexesSortable42, (int x, int y) => inv[x].hammer.CompareTo(inv[y].hammer));
			return indexesSortable42;
		});
		...
		{
			List<int> indexesSortable41 = itemsToSort.Where((int i) => inv[i].type > 0 && ItemID.Sets.SortingPriorityTerraforming[inv[i].type] > -1).ToList();
			layer.Validate(ref indexesSortable41, inv);
			foreach (int item12 in indexesSortable41)
			{
				itemsToSort.Remove(item12);
			}
			SortIndicesStable(indexesSortable41, delegate(int x, int y)
			{
				int num39 = ItemID.Sets.SortingPriorityTerraforming[inv[x].type].CompareTo(ItemID.Sets.SortingPriorityTerraforming[inv[y].type]);
				if (num39 == 0)
				{
					num39 = inv[y].stack.CompareTo(inv[x].stack);
				}
				return num39;
			});
			return indexesSortable41;
		});
		...
		{
			List<int> indexesSortable40 = itemsToSort.Where((int i) => inv[i].fishingPole > 0 || inv[i].bait > 0 || inv[i].questItem || (inv[i].type > 0 && (ItemID.Sets.IsFishingCrate[inv[i].type] || ItemID.Sets.IsBasicFish[inv[i].type] || ItemID.Sets.SortingPriorityToolsFishing[inv[i].type] > -1))).ToList();
			layer.Validate(ref indexesSortable40, inv);
			foreach (int item13 in indexesSortable40)
			{
				itemsToSort.Remove(item13);
			}
			SortIndicesStable(indexesSortable40, delegate(int x, int y)
			{
				int num38 = CompareWithPrioritySet(ItemID.Sets.SortingPriorityToolsFishing, inv[x].type, inv[y].type);
				if (num38 == 0)
				{
					num38 = inv[y].fishingPole.CompareTo(inv[x].fishingPole);
				}
				if (num38 == 0)
				{
					num38 = inv[y].bait.CompareTo(inv[x].bait);
				}
				if (num38 == 0)
				{
					num38 = inv[y].questItem.CompareTo(inv[x].questItem);
				}
				if (num38 == 0 && inv[y].type >= 0 && inv[x].type >= 0)
				{
					if (num38 == 0)
					{
						num38 = ItemID.Sets.IsFishingCrate[inv[y].type].CompareTo(ItemID.Sets.IsFishingCrate[inv[x].type]);
					}
					if (num38 == 0)
					{
						num38 = ItemID.Sets.IsBasicFish[inv[y].type].CompareTo(ItemID.Sets.IsBasicFish[inv[x].type]);
					}
				}
				if (num38 == 0)
				{
					num38 = inv[y].OriginalRarity.CompareTo(inv[x].OriginalRarity);
				}
				if (num38 == 0)
				{
					num38 = inv[y].stack.CompareTo(inv[x].stack);
				}
				return num38;
			});
			return indexesSortable40;
		});
		...
		{
			List<int> indexesSortable39 = itemsToSort.Where((int i) => inv[i].type > 0 && ItemID.Sets.SortingPriorityToolsGolf[inv[i].type] > -1).ToList();
			layer.Validate(ref indexesSortable39, inv);
			foreach (int item14 in indexesSortable39)
			{
				itemsToSort.Remove(item14);
			}
			SortIndicesStable(indexesSortable39, delegate(int x, int y)
			{
				int num37 = ItemID.Sets.SortingPriorityToolsGolf[inv[x].type].CompareTo(ItemID.Sets.SortingPriorityToolsGolf[inv[y].type]);
				if (num37 == 0)
				{
					num37 = inv[y].OriginalRarity.CompareTo(inv[x].OriginalRarity);
				}
				if (num37 == 0)
				{
					num37 = inv[y].stack.CompareTo(inv[x].stack);
				}
				return num37;
			});
			return indexesSortable39;
		});
		...
		{
			List<int> indexesSortable38 = itemsToSort.Where((int i) => inv[i].type > 0 && ItemID.Sets.SortingPriorityToolsInstruments[inv[i].type] > -1).ToList();
			layer.Validate(ref indexesSortable38, inv);
			foreach (int item15 in indexesSortable38)
			{
				itemsToSort.Remove(item15);
			}
			SortIndicesStable(indexesSortable38, delegate(int x, int y)
			{
				int num36 = ItemID.Sets.SortingPriorityToolsInstruments[inv[x].type].CompareTo(ItemID.Sets.SortingPriorityToolsInstruments[inv[y].type]);
				if (num36 == 0)
				{
					num36 = inv[y].OriginalRarity.CompareTo(inv[x].OriginalRarity);
				}
				if (num36 == 0)
				{
					num36 = inv[y].stack.CompareTo(inv[x].stack);
				}
				return num36;
			});
		...
		{
			List<int> indexesSortable37 = itemsToSort.Where((int i) => inv[i].type > 0 && ItemID.Sets.SortingPriorityToolsKeys[inv[i].type] > -1).ToList();
			layer.Validate(ref indexesSortable37, inv);
			foreach (int item16 in indexesSortable37)
			{
				itemsToSort.Remove(item16);
			}
			SortIndicesStable(indexesSortable37, delegate(int x, int y)
			{
				int num35 = ItemID.Sets.SortingPriorityToolsKeys[inv[x].type].CompareTo(ItemID.Sets.SortingPriorityToolsKeys[inv[y].type]);
				if (num35 == 0)
				{
					num35 = inv[y].OriginalRarity.CompareTo(inv[x].OriginalRarity);
				}
				if (num35 == 0)
				{
					num35 = inv[y].stack.CompareTo(inv[x].stack);
				}
				return num35;
			});
		...
		{
			List<int> indexesSortable36 = itemsToSort.Where((int i) => inv[i].type > 0 && ItemID.Sets.SortingPriorityToolsKites[inv[i].type] > -1).ToList();
			layer.Validate(ref indexesSortable36, inv);
			foreach (int item17 in indexesSortable36)
			{
				itemsToSort.Remove(item17);
			}
			SortIndicesStable(indexesSortable36, delegate(int x, int y)
			{
				int num34 = ItemID.Sets.SortingPriorityToolsKites[inv[x].type].CompareTo(ItemID.Sets.SortingPriorityToolsKites[inv[y].type]);
				if (num34 == 0)
				{
					num34 = inv[y].OriginalRarity.CompareTo(inv[x].OriginalRarity);
				}
				if (num34 == 0)
				{
					num34 = inv[y].stack.CompareTo(inv[x].stack);
				}
				return num34;
			});
		...
		{
			List<int> indexesSortable35 = itemsToSort.Where((int i) => inv[i].ammo > 0 && inv[i].type >= 0 && inv[i].type < ItemID.Count && !ItemID.Sets.IsFood[inv[i].type] && ItemID.Sets.SortingPriorityMiscAcorns[inv[i].type] == -1).ToList();
			layer.Validate(ref indexesSortable35, inv);
			foreach (int item18 in indexesSortable35)
			{
				itemsToSort.Remove(item18);
			}
			SortIndicesStable(indexesSortable35, delegate(int x, int y)
			{
		...
		{
			List<int> indexesSortable34 = itemsToSort.Where((int i) => inv[i].type > 0 && ItemID.Sets.SortingPriorityToolsMisc[inv[i].type] > -1).ToList();
			layer.Validate(ref indexesSortable34, inv);
			foreach (int item19 in indexesSortable34)
			{
				itemsToSort.Remove(item19);
			}
			SortIndicesStable(indexesSortable34, delegate(int x, int y)
			{
				int num32 = ItemID.Sets.SortingPriorityToolsMisc[inv[x].type].CompareTo(ItemID.Sets.SortingPriorityToolsMisc[inv[y].type]);
				if (num32 == 0)
				{
					num32 = inv[y].OriginalRarity.CompareTo(inv[x].OriginalRarity);
				}
				if (num32 == 0)
				{
					num32 = inv[y].stack.CompareTo(inv[x].stack);
				}
				return num32;
			});
		...
		{
			List<int> indexesSortable33 = itemsToSort.Where((int i) => (inv[i].bodySlot >= 0 || inv[i].headSlot >= 0 || inv[i].legSlot >= 0) && !inv[i].vanity).ToList();
			layer.Validate(ref indexesSortable33, inv);
			foreach (int item20 in indexesSortable33)
			{
				itemsToSort.Remove(item20);
			}
			SortIndicesStable(indexesSortable33, delegate(int x, int y)
			{
				int num31 = inv[y].OriginalRarity.CompareTo(inv[x].OriginalRarity);
				if (num31 == 0)
				{
					num31 = inv[y].OriginalDefense.CompareTo(inv[x].OriginalDefense);
				}
				if (num31 == 0)
				{
					num31 = inv[x].type.CompareTo(inv[y].type);
				}
				return num31;
			});
		...
		{
			List<int> indexesSortable32 = itemsToSort.Where((int i) => (inv[i].bodySlot >= 0 || inv[i].headSlot >= 0 || inv[i].legSlot >= 0) && inv[i].vanity).ToList();
			layer.Validate(ref indexesSortable32, inv);
			foreach (int item21 in indexesSortable32)
			{
				itemsToSort.Remove(item21);
			}
			SortIndicesStable(indexesSortable32, delegate(int x, int y)
			{
				int num30 = inv[y].OriginalRarity.CompareTo(inv[x].OriginalRarity);
				if (num30 == 0)
				{
					num30 = inv[x].type.CompareTo(inv[y].type);
				}
				return num30;
			});
		...
		{
			List<int> indexesSortable31 = itemsToSort.Where((int i) => inv[i].accessory).ToList();
			layer.Validate(ref indexesSortable31, inv);
			foreach (int item22 in indexesSortable31)
			{
				itemsToSort.Remove(item22);
			}
			SortIndicesStable(indexesSortable31, delegate(int x, int y)
			{
				int num29 = inv[x].vanity.CompareTo(inv[y].vanity);
				if (num29 == 0)
				{
				...
				if (num29 == 0)
				{
					num29 = inv[y].OriginalDefense.CompareTo(inv[x].OriginalDefense);
				}
				if (num29 == 0)
				{
					num29 = inv[x].type.CompareTo(inv[y].type);
				}
				return num29;
			});
		...
		{
			List<int> indexesSortable30 = itemsToSort.Where((int i) => Main.projHook[inv[i].shoot]).ToList();
			layer.Validate(ref indexesSortable30, inv);
			foreach (int item23 in indexesSortable30)
			{
				itemsToSort.Remove(item23);
			}
			SortIndicesStable(indexesSortable30, delegate(int x, int y)
			{
				int num28 = inv[y].OriginalRarity.CompareTo(inv[x].OriginalRarity);
				if (num28 == 0)
				{
					num28 = inv[x].type.CompareTo(inv[y].type);
				}
				return num28;
			});
		...
		{
			List<int> indexesSortable29 = itemsToSort.Where((int i) => inv[i].mountType != -1 && !MountID.Sets.Cart[inv[i].mountType]).ToList();
			layer.Validate(ref indexesSortable29, inv);
			foreach (int item24 in indexesSortable29)
			{
				itemsToSort.Remove(item24);
			}
			SortIndicesStable(indexesSortable29, delegate(int x, int y)
			{
				int num27 = inv[y].OriginalRarity.CompareTo(inv[x].OriginalRarity);
				if (num27 == 0)
				{
					num27 = inv[x].type.CompareTo(inv[y].type);
				}
				return num27;
			});
		...
		{
			List<int> indexesSortable28 = itemsToSort.Where((int i) => inv[i].mountType != -1 && MountID.Sets.Cart[inv[i].mountType]).ToList();
			layer.Validate(ref indexesSortable28, inv);
			foreach (int item25 in indexesSortable28)
			{
				itemsToSort.Remove(item25);
			}
			SortIndicesStable(indexesSortable28, delegate(int x, int y)
			{
				int num26 = inv[y].OriginalRarity.CompareTo(inv[x].OriginalRarity);
				if (num26 == 0)
				{
					num26 = inv[x].type.CompareTo(inv[y].type);
				}
				return num26;
			});
		...
		{
			List<int> indexesSortable27 = itemsToSort.Where((int i) => inv[i].buffType > 0 && Main.lightPet[inv[i].buffType]).ToList();
			layer.Validate(ref indexesSortable27, inv);
			foreach (int item26 in indexesSortable27)
			{
				itemsToSort.Remove(item26);
			}
			SortIndicesStable(indexesSortable27, delegate(int x, int y)
			{
				int num25 = inv[y].OriginalRarity.CompareTo(inv[x].OriginalRarity);
				if (num25 == 0)
				{
					num25 = inv[x].type.CompareTo(inv[y].type);
				}
				return num25;
			});
		...
		{
			List<int> indexesSortable26 = itemsToSort.Where((int i) => inv[i].buffType > 0 && Main.vanityPet[inv[i].buffType]).ToList();
			layer.Validate(ref indexesSortable26, inv);
			foreach (int item27 in indexesSortable26)
			{
				itemsToSort.Remove(item27);
			}
			SortIndicesStable(indexesSortable26, delegate(int x, int y)
			{
				int num24 = inv[y].OriginalRarity.CompareTo(inv[x].OriginalRarity);
				if (num24 == 0)
				{
					num24 = inv[x].type.CompareTo(inv[y].type);
				}
				return num24;
			});
		...
		{
			List<int> indexesSortable25 = itemsToSort.Where((int i) => inv[i].consumable && inv[i].healLife > 0 && inv[i].healMana < 1 && inv[i].type != 5).ToList();
			layer.Validate(ref indexesSortable25, inv);
			foreach (int item28 in indexesSortable25)
			{
				itemsToSort.Remove(item28);
			}
			SortIndicesStable(indexesSortable25, delegate(int x, int y)
			{
				int num23 = inv[y].healLife.CompareTo(inv[x].healLife);
				if (num23 == 0)
				{
		...
		{
			List<int> indexesSortable24 = itemsToSort.Where((int i) => inv[i].type == 5).ToList();
			layer.Validate(ref indexesSortable24, inv);
			foreach (int item29 in indexesSortable24)
			{
				itemsToSort.Remove(item29);
			}
			SortIndicesStable(indexesSortable24, (int x, int y) => inv[y].stack.CompareTo(inv[x].stack));
		...
		{
			List<int> indexesSortable23 = itemsToSort.Where((int i) => inv[i].consumable && inv[i].healLife < 1 && inv[i].healMana > 0).ToList();
			layer.Validate(ref indexesSortable23, inv);
			foreach (int item30 in indexesSortable23)
			{
				itemsToSort.Remove(item30);
			}
			SortIndicesStable(indexesSortable23, delegate(int x, int y)
			{
				int num22 = inv[y].healMana.CompareTo(inv[x].healMana);
				if (num22 == 0)
				{
					num22 = inv[y].stack.CompareTo(inv[x].stack);
				}
				return num22;
			});
		...
		{
			List<int> indexesSortable22 = itemsToSort.Where((int i) => inv[i].consumable && inv[i].healLife > 0 && inv[i].healMana > 0).ToList();
			layer.Validate(ref indexesSortable22, inv);
			foreach (int item31 in indexesSortable22)
			{
				itemsToSort.Remove(item31);
			}
			SortIndicesStable(indexesSortable22, delegate(int x, int y)
			{
				int num21 = inv[y].healLife.CompareTo(inv[x].healLife);
				if (num21 == 0)
				{
		...
		{
			List<int> indexesSortable21 = itemsToSort.Where((int i) => (inv[i].consumable && inv[i].buffType > 0 && inv[i].type >= 0 && inv[i].type < ItemID.Count && !ItemID.Sets.IsFood[inv[i].type]) || (inv[i].type >= 0 && ItemID.Sets.SortingPriorityPotionsBuffs[inv[i].type] > -1)).ToList();
			layer.Validate(ref indexesSortable21, inv);
			foreach (int item32 in indexesSortable21)
			{
				itemsToSort.Remove(item32);
			}
			SortIndicesStable(indexesSortable21, delegate(int x, int y)
			{
				int num20 = CompareWithPrioritySet(ItemID.Sets.SortingPriorityPotionsBuffs, inv[x].type, inv[y].type);
				if (num20 == 0)
				{
					num20 = inv[y].OriginalRarity.CompareTo(inv[x].OriginalRarity);
				}
				if (num20 == 0)
				{
					num20 = inv[x].type.CompareTo(inv[y].type);
				}
				if (num20 == 0)
				{
					num20 = inv[y].stack.CompareTo(inv[x].stack);
				}
				return num20;
			});
		...
		{
			List<int> indexesSortable20 = itemsToSort.Where((int i) => inv[i].consumable && inv[i].buffType > 0 && inv[i].type >= 0 && inv[i].type < ItemID.Count && ItemID.Sets.IsFood[inv[i].type]).ToList();
			layer.Validate(ref indexesSortable20, inv);
			foreach (int item33 in indexesSortable20)
			{
				itemsToSort.Remove(item33);
			}
			SortIndicesStable(indexesSortable20, delegate(int x, int y)
			{
				int num19 = ((inv[y].buffType >= 0 && inv[y].buffType < BuffID.Count) ? BuffID.Sets.SortingPriorityFoodBuffs[inv[y].buffType].CompareTo(BuffID.Sets.SortingPriorityFoodBuffs[inv[x].buffType]) : 0);
				if (num19 == 0)
				{
					num19 = inv[y].OriginalRarity.CompareTo(inv[x].OriginalRarity);
				}
				if (num19 == 0)
				{
					num19 = inv[x].type.CompareTo(inv[y].type);
				}
				if (num19 == 0)
				{
		...
		{
			List<int> indexesSortable19 = itemsToSort.Where((int i) => inv[i].dye > 0 || (inv[i].type >= 0 && ItemID.Sets.SortingPriorityPotionsDyeMaterial[inv[i].type] > -1)).ToList();
			layer.Validate(ref indexesSortable19, inv);
			foreach (int item34 in indexesSortable19)
			{
				itemsToSort.Remove(item34);
			}
			SortIndicesStable(indexesSortable19, delegate(int x, int y)
			{
				int num18 = inv[y].OriginalRarity.CompareTo(inv[x].OriginalRarity);
				if (num18 == 0)
				{
					num18 = inv[y].dye.CompareTo(inv[x].dye);
				}
				if (num18 == 0)
				{
					num18 = CompareWithPrioritySet(ItemID.Sets.SortingPriorityPotionsDyeMaterial, inv[x].type, inv[y].type);
				}
				if (num18 == 0)
				{
					num18 = inv[y].stack.CompareTo(inv[x].stack);
				}
				return num18;
			});
		...
		{
			List<int> indexesSortable18 = itemsToSort.Where((int i) => inv[i].hairDye >= 0).ToList();
			layer.Validate(ref indexesSortable18, inv);
			foreach (int item35 in indexesSortable18)
			{
				itemsToSort.Remove(item35);
			}
			SortIndicesStable(indexesSortable18, delegate(int x, int y)
			{
				int num17 = inv[y].OriginalRarity.CompareTo(inv[x].OriginalRarity);
				if (num17 == 0)
				{
					num17 = inv[y].hairDye.CompareTo(inv[x].hairDye);
				}
				if (num17 == 0)
				{
		...
		{
			List<int> indexesSortable17 = itemsToSort.Where((int i) => inv[i].type > 0 && ItemID.Sets.SortingPriorityMiscImportants[inv[i].type] > -1).ToList();
			layer.Validate(ref indexesSortable17, inv);
			foreach (int item36 in indexesSortable17)
			{
				itemsToSort.Remove(item36);
			}
			SortIndicesStable(indexesSortable17, delegate(int x, int y)
			{
				int num16 = ItemID.Sets.SortingPriorityMiscImportants[inv[x].type].CompareTo(ItemID.Sets.SortingPriorityMiscImportants[inv[y].type]);
				if (num16 == 0)
				{
		...
		{
			List<int> indexesSortable16 = itemsToSort.Where((int i) => (inv[i].type > 0 && ItemID.Sets.SortingPriorityWiring[inv[i].type] > -1) || inv[i].mech).ToList();
			layer.Validate(ref indexesSortable16, inv);
			foreach (int item37 in indexesSortable16)
			{
				itemsToSort.Remove(item37);
			}
			SortIndicesStable(indexesSortable16, delegate(int x, int y)
			{
				int num15 = ItemID.Sets.SortingPriorityWiring[inv[y].type].CompareTo(ItemID.Sets.SortingPriorityWiring[inv[x].type]);
				if (num15 == 0)
				{
					num15 = inv[y].OriginalRarity.CompareTo(inv[x].OriginalRarity);
				}
				if (num15 == 0)
				{
					num15 = inv[y].type.CompareTo(inv[x].type);
				}
				if (num15 == 0)
				{
		...
		{
			List<int> indexesSortable15 = itemsToSort.Where((int i) => inv[i].type > 0 && ItemID.Sets.SortingPriorityMaterials[inv[i].type] > -1).ToList();
			layer.Validate(ref indexesSortable15, inv);
			foreach (int item38 in indexesSortable15)
			{
				itemsToSort.Remove(item38);
			}
			SortIndicesStable(indexesSortable15, delegate(int x, int y)
			{
				int num14 = ItemID.Sets.SortingPriorityMaterials[inv[y].type].CompareTo(ItemID.Sets.SortingPriorityMaterials[inv[x].type]);
				if (num14 == 0)
				{
		...
		{
			List<int> indexesSortable14 = itemsToSort.Where((int i) => inv[i].type == 183).ToList();
			layer.Validate(ref indexesSortable14, inv);
			foreach (int item39 in indexesSortable14)
			{
				itemsToSort.Remove(item39);
			}
			SortIndicesStable(indexesSortable14, (int x, int y) => inv[y].stack.CompareTo(inv[x].stack));
		...
		{
			List<int> indexesSortable13 = itemsToSort.Where((int i) => inv[i].type > 0 && ItemID.Sets.SortingPriorityExtractibles[inv[i].type] > -1).ToList();
			layer.Validate(ref indexesSortable13, inv);
			foreach (int item40 in indexesSortable13)
			{
				itemsToSort.Remove(item40);
			}
			SortIndicesStable(indexesSortable13, delegate(int x, int y)
			{
				int num13 = ItemID.Sets.SortingPriorityExtractibles[inv[y].type].CompareTo(ItemID.Sets.SortingPriorityExtractibles[inv[x].type]);
				if (num13 == 0)
				{
		...
		{
			List<int> indexesSortable12 = itemsToSort.Where((int i) => (inv[i].type > 0 && ItemID.Sets.SortingPriorityPainting[inv[i].type] > -1) || inv[i].paint > 0).ToList();
			layer.Validate(ref indexesSortable12, inv);
			foreach (int item41 in indexesSortable12)
			{
				itemsToSort.Remove(item41);
			}
			SortIndicesStable(indexesSortable12, delegate(int x, int y)
			{
				int num12 = ItemID.Sets.SortingPriorityPainting[inv[y].type].CompareTo(ItemID.Sets.SortingPriorityPainting[inv[x].type]);
				if (num12 == 0)
				{
					num12 = inv[x].paint.CompareTo(inv[y].paint);
				}
				if (num12 == 0)
				{
					num12 = inv[x].paintCoating.CompareTo(inv[y].paintCoating);
				}
				if (num12 == 0)
				{
		...
		{
			List<int> indexesSortable11 = itemsToSort.Where((int i) => inv[i].type > 0 && ItemID.Sets.SortingPriorityRopes[inv[i].type] > -1).ToList();
			layer.Validate(ref indexesSortable11, inv);
			foreach (int item42 in indexesSortable11)
			{
				itemsToSort.Remove(item42);
			}
			SortIndicesStable(indexesSortable11, delegate(int x, int y)
			{
				int num11 = ItemID.Sets.SortingPriorityRopes[inv[y].type].CompareTo(ItemID.Sets.SortingPriorityRopes[inv[x].type]);
				if (num11 == 0)
				{
		...
		{
			List<int> indexesSortable10 = itemsToSort.Where((int i) => inv[i].type > 0 && ItemID.Sets.SortingPriorityMiscHerbsAndSeeds[inv[i].type] > -1).ToList();
			layer.Validate(ref indexesSortable10, inv);
			foreach (int item43 in indexesSortable10)
			{
				itemsToSort.Remove(item43);
			}
			SortIndicesStable(indexesSortable10, delegate(int x, int y)
			{
				int num10 = ItemID.Sets.SortingPriorityMiscHerbsAndSeeds[inv[y].type].CompareTo(ItemID.Sets.SortingPriorityMiscHerbsAndSeeds[inv[x].type]);
				if (num10 == 0)
				{
		...
		{
			List<int> indexesSortable9 = itemsToSort.Where((int i) => inv[i].type > 0 && ItemID.Sets.SortingPriorityMiscGems[inv[i].type] > -1).ToList();
			layer.Validate(ref indexesSortable9, inv);
			foreach (int item44 in indexesSortable9)
			{
				itemsToSort.Remove(item44);
			}
			SortIndicesStable(indexesSortable9, delegate(int x, int y)
			{
				int num9 = ItemID.Sets.SortingPriorityMiscGems[inv[y].type].CompareTo(ItemID.Sets.SortingPriorityMiscGems[inv[x].type]);
				if (num9 == 0)
				{
		...
		{
			List<int> indexesSortable8 = itemsToSort.Where((int i) => inv[i].type > 0 && ItemID.Sets.SortingPriorityMiscAcorns[inv[i].type] > -1).ToList();
			layer.Validate(ref indexesSortable8, inv);
			foreach (int item45 in indexesSortable8)
			{
				itemsToSort.Remove(item45);
			}
			SortIndicesStable(indexesSortable8, delegate(int x, int y)
			{
				int num8 = ItemID.Sets.SortingPriorityMiscAcorns[inv[y].type].CompareTo(ItemID.Sets.SortingPriorityMiscAcorns[inv[x].type]);
				if (num8 == 0)
				{
		...
		{
			List<int> indexesSortable7 = itemsToSort.Where((int i) => inv[i].type > 0 && ItemID.Sets.SortingPriorityMiscBossBags[inv[i].type] > -1).ToList();
			layer.Validate(ref indexesSortable7, inv);
			foreach (int item46 in indexesSortable7)
			{
				itemsToSort.Remove(item46);
			}
			SortIndicesStable(indexesSortable7, delegate(int x, int y)
			{
				int num7 = ItemID.Sets.SortingPriorityMiscBossBags[inv[x].type].CompareTo(ItemID.Sets.SortingPriorityMiscBossBags[inv[y].type]);
				if (num7 == 0)
				{
					num7 = inv[y].OriginalRarity.CompareTo(inv[x].OriginalRarity);
				}
				if (num7 == 0)
				{
		...
		{
			List<int> indexesSortable6 = itemsToSort.Where((int i) => inv[i].makeNPC > 0).ToList();
			layer.Validate(ref indexesSortable6, inv);
			foreach (int item47 in indexesSortable6)
			{
				itemsToSort.Remove(item47);
			}
			SortIndicesStable(indexesSortable6, delegate(int x, int y)
			{
				int num6 = inv[x].makeNPC.CompareTo(inv[y].makeNPC);
				if (num6 == 0)
				{
		...
		{
			List<int> indexesSortable5 = itemsToSort.Where((int i) => inv[i].createTile < 0 && inv[i].createWall < 1 && inv[i].rare != -1).ToList();
			layer.Validate(ref indexesSortable5, inv);
			foreach (int item48 in indexesSortable5)
			{
				itemsToSort.Remove(item48);
			}
			SortIndicesStable(indexesSortable5, delegate(int x, int y)
			{
		...
		public static ItemSortingLayer LastTilesImportant = new ItemSortingLayer("Last - Tiles (Frame Important)", delegate(ItemSortingLayer layer, Item[] inv, List<int> itemsToSort)
		{
			...
			foreach (int item49 in indexesSortable4)
			{
				itemsToSort.Remove(item49);
			}
			SortIndicesStable(indexesSortable4, delegate(int x, int y)
			{
		...
		public static ItemSortingLayer LastTilesCommon = new ItemSortingLayer("Last - Tiles (Common), Walls", delegate(ItemSortingLayer layer, Item[] inv, List<int> itemsToSort)
		{
			...
			foreach (int item50 in indexesSortable3)
			{
				itemsToSort.Remove(item50);
			}
			SortIndicesStable(indexesSortable3, delegate(int x, int y)
			{
		...
		public static ItemSortingLayer LastNotTrash = new ItemSortingLayer("Last - Not Trash", delegate(ItemSortingLayer layer, Item[] inv, List<int> itemsToSort)
		{
			...
			foreach (int item51 in indexesSortable2)
			{
				itemsToSort.Remove(item51);
			}
			SortIndicesStable(indexesSortable2, delegate(int x, int y)
			{
