...
public class CraftingUI : ICraftingUI
{
	...
	public static NewCraftingUI.RecipeFilter RecipeFilterHack
	{
		get
		{
			if (!Main.playerInventory || Main.PipsCurrentPage != 0 || Player.Settings.CraftingGridControl != Player.Settings.CraftingGridMode.Classic)
			{
				return null;
			}
			return _lastFilter;
	...
	{
		if (_lastFilter != null && (!_lastFilter.CanRemainOpen() || Main.PipsCurrentPage != 0 || !Main.playerInventory))
		{
			_lastFilter = null;
		}
	...
	public void DrawRecipesGrid(SpriteBatch spriteBatch)
	{
		...
		if (recStart > 0)
		{
			...
			spriteBatch.Draw(TextureAssets.CraftUpButton.Value, new Vector2(num10, num11), null, new Color((byte)200, (byte)200, (byte)200, (byte)200), 0f, default(Vector2), 1f, SpriteEffects.None, 0f);
		}
		if (recStart < numAvailableRecipes - num4 * num5)
		{
			num11 += 20;
			...
			spriteBatch.Draw(TextureAssets.CraftDownButton.Value, new Vector2(num10, num11), null, new Color((byte)200, (byte)200, (byte)200, (byte)200), 0f, default(Vector2), 1f, SpriteEffects.None, 0f);
		}
