...
public class LeashedKite : LeashedEntity
{
	...
	private void MoveKite(bool fastForward = false)
	{
		...
		timeCounter += 1f / 60f;
		KiteFlyingInfo kiteFlyingInfo = default(KiteFlyingInfo);
		kiteFlyingInfo.BobOffset = (vector.X + vector.Y * 0.92f) * 0.0025f;
		kiteFlyingInfo.WindInWorld = windCurrent;
		kiteFlyingInfo.CloudAlpha = cloudAlpha;
		kiteFlyingInfo.GlobalTime = timeCounter;
		kiteFlyingInfo.CanReelThroughBlocks = false;
		KiteFlyingInfo info = kiteFlyingInfo;
		if (fastForward)
		{
