	...
	private void UpdateDeviceList(List<RgbDevice> deviceList)
	{
		foreach (RgbDevice device in deviceList)
		{
			...
			for (int i = 0; i < list.Count; i++)
			{
				...
				if (jObject != null)
				{
					jObject.Add("game", _gameSenseConnection.GameName);
					string fullEventJson = jObject.ToString(Formatting.None);
		...
		foreach (IGameSenseUpdater miscEvent in _miscEvents)
		{
			...
			for (int j = 0; j < list2.Count; j++)
			{
				...
				if (jObject2 != null)
				{
					jObject2.Add("game", _gameSenseConnection.GameName);
					string fullEventJson2 = jObject2.ToString(Formatting.None);
